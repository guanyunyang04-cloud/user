...
class FrontierScannerTest(unittest.TestCase):
    ...
    def test_output_newer_than_brain_sets_stale_warning(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_json(
                summary,
                {"study_tag": "path20_new", "status": "completed", "evidence_verdict": "forecast_test_confirmed"},
            )
            ...
            self._write_json(registry, {"records": [{"id": "path20_new", "study_tags": ["path20_new"]}]})
            self._touch(ref, 100)
    ...
    def test_failed_or_interrupted_summary_is_not_upgraded_to_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_json(
                summary,
                {"study_tag": "path20_failed", "status": "failed", "evidence_verdict": "forecast_failed"},
            )
            ref = root / "daily_research/brain/references/current.md"
            ref.parent.mkdir(parents=True, exist_ok=True)
            ref.write_text("# current\n", encoding="utf-8")
            registry = root / "daily_research/brain/references/evidence_registry.json"
            self._write_json(registry, {"records": [{"study_tags": ["path20_failed"]}]})

            report = build_frontier_report(workspace_root=root)

        latest = report["latest_output_studies"][0]
    ...
    def test_registered_output_with_newer_brain_reference_is_not_stale(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_json(summary, {"study_tag": "path20_registered", "status": "completed"})
            ref = root / "daily_research/brain/references/current.md"
            ref.parent.mkdir(parents=True, exist_ok=True)
            ref.write_text("# newer\n", encoding="utf-8")
            registry = root / "daily_research/brain/references/evidence_registry.json"
            self._write_json(registry, {"records": [{"study_tags": ["path20_registered"]}]})
            self._touch(summary, 100)
            self._touch(ref, 200)
            self._touch(registry, 200)
        ...
        self.assertEqual(report["unregistered_latest_tags"], [])
