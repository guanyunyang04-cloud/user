...
ACTIVE_ARTIFACT = daily_research_adapter.ACTIVE_ARTIFACT
...
def finding_for_frontier_staleness() -> BrainRuleFinding | None:
    report = daily_research_adapter.build_frontier_report()
...
def findings_for_control_plane_doc_lengths(
    ...
    findings: list[BrainRuleFinding] = []
    for rel_path, max_lines in (limits or daily_research_adapter.control_plane_doc_limits()).items():
