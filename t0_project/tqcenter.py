...
def _find_tq_dll(start_path: Path) -> Path:
    # Search upward for TPythClient.dll to keep tqcenter location flexible.
    start_path = start_path.resolve()
    search_roots = [start_path.parent] + list(start_path.parents)
    for parent in search_roots:
        candidate = parent / 'TPythClient.dll'
        if candidate.exists():
            return candidate
    raise FileNotFoundError(
        "TPythClient.dll not found. Please ensure it exists under the TDX installation path."
    )

global_dll_path = _find_tq_dll(Path(__file__))
