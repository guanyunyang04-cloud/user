...
def _normalize_industry_comparison(value: Any) -> str:
    return re.sub(r"^[a-z]\d{1,4}", "", _normalize_comparison_text(value))
...
def _normalize_cninfo_industry_history(
    frame: pd.DataFrame,
    *,
    symbol: str,
    target_date: str,
...
def _normalize_daily_basic(frame: pd.DataFrame, trade_date: str) -> pd.DataFrame:
    data = frame.copy()
...
def _normalize_cninfo_share_change(
    frame: pd.DataFrame,
    *,
    symbol: str,
    target_date: str,
...
def _normalize_dividend(frame: pd.DataFrame, *, target_date: str) -> pd.DataFrame:
    ...
    data["ex_date"] = _tushare_date_series(data, "ex_date")
...
def _normalize_cninfo_dividend(
    frame: pd.DataFrame,
    *,
    symbol: str,
    target_date: str,
