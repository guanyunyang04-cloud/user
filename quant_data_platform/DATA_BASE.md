...
- QDP v3 代码已切换到可信单源合同：Tushare-compatible proxy 独占 `2010-01-01..2026-07-13` 历史启动，历史 5m 优先使用每日分钟额度；BaoStock 历史复刻与跨源逐行验证已退出生产 DAG。2026-07-15 已恢复历史 5m 后台 DAG；它按页级 staging 与单证券 raw 断点续跑，额度耗尽后只补 `status/factor`。v3 candidate、audit 和 publish 仍未完成。
