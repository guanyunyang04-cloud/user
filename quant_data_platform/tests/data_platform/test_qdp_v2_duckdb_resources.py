...
def test_dynamic_memory_limit_has_no_two_gib_cap() -> None:
    ...
    assert limit == 16 * 1024**3 - DEFAULT_MEMORY_FLOOR_BYTES
    assert limit > 2 * 1024**3


def test_guarded_connection_uses_total_ram_ceiling(tmp_path) -> None:
    with open_guarded_duckdb(
        ...
        assert connection.settings.memory_limit_bytes == 16642998272
        assert connection.execute("SELECT 42").fetchone() == (42,)


def test_watchdog_requires_five_continuously_low_seconds() -> None:
    ...
    assert watchdog.sample_once(available_bytes=600, now=4.0) is False
    assert watchdog.sample_once(available_bytes=400, now=10.0) is False
    assert watchdog.sample_once(available_bytes=400, now=14.99) is False
    assert connection.interrupts == 0

    assert watchdog.sample_once(available_bytes=400, now=15.0) is True
...
def test_watchdog_interrupts_only_once_after_trigger() -> None:
    ...
    watchdog.sample_once(available_bytes=400, now=6.0)
    watchdog.sample_once(available_bytes=300, now=7.0)
