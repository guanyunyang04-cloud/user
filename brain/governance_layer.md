...
- 普通读写、短脚本、一次性诊断、测试、报告、临时 JSON、日志、截图、提交和进程管理都受同一项目任务命名空间约束；长任务只是其中的受监管特例，不是唯一需要隔离的任务。
...
- `long_task_monitor` 必须携带 `--project-id` 与 `--run-id`；没有显式 active cross-project lease 时，其他项目 agent 不得 wait、tail/read 日志产物或管理该进程。
