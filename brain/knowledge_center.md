...
- `no-codex-thread-deeplinks`
  - Codex 线程 URI 属于会话运行时定位，不是项目长期记忆；brain 不维护“每个任务一个线程深链”的分配表。
  - 验收时不再单独运行全仓深链搜索步骤；由 `doc_guard.py check` 的 thread-deeplink 守卫统一覆盖。
...
- 如果把任务接力依赖在线程深链上，后续 agent 会被绑定到不可审计、不可迁移的会话状态；长期接管必须回到 brain 路由、分脑状态和产物路径
