...
from .sampling import calibrate_groups_per_day, event_query
from .source import (
    SourceSnapshot,
...
CHECKPOINT_SCHEMA = "quantlab.minute_v2_day_parts/2"
...
def _verify_month_artifacts(
    ...
    *,
    ...
    if sample_groups != int(expected_sample_groups):
        raise MinuteV2Error(
            f"minute_v2_sample_group_count_mismatch:{sample_groups}:{expected_sample_groups}"
        )
    ...
    if incomplete_groups:
        raise MinuteV2Error(f"minute_v2_incomplete_cross_section_groups:{incomplete_groups}")
    ...
    return {
        ...
        "sample_group_count": sample_groups,
        "incomplete_sample_group_count": incomplete_groups,
...
def build_month(
    ...
    *,
    ...
    try:
        ...
        sampling_calibration = calibrate_groups_per_day(
            base_rows=base_rows,
            complete_group_count=int(group_stats[0]),
            selected_trading_days=int(group_stats[1]),
            uncompressed_bytes=uncompressed_base_bytes,
            total_memory_bytes=int(psutil.virtual_memory().total),
            memory_floor_bytes=int(current.memory_floor_gib * GIB),
            memory_fraction=float(current.training_sample_memory_fraction),
            maximum_groups_per_day=int(group_stats[2]),
        )
        groups_per_day = int(sampling_calibration["groups_per_day"])
        expected_sample_groups = groups_per_day * int(group_stats[1])
        sample_phase_started = time.perf_counter()
        for chunk_dates, base_part in zip(processing_chunks, base_parts, strict=True):
            ...
            if not (need_events or need_labels):
                reused_parts += 2
                continue
            connection.execute(
                "CREATE OR REPLACE TEMP VIEW day_minute_bars AS "
        ...
        sample_phase_seconds = time.perf_counter() - sample_phase_started
        ...
        verification = _verify_month_artifacts(
            ...
            expected_sample_groups=expected_sample_groups,
        ...
        benchmark = {
            ...
            "sample_and_label_seconds": sample_phase_seconds,
            ...
            "sampling_calibration": sampling_calibration,
        }
        result = {
            ...
            "storage_policy": (
                ...
                if keep_base
                else "complete causal base computed transiently; measured whole cross-sections retained"
            ),
            "benchmark": benchmark,
