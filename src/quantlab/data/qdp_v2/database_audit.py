...
from quantlab.data.qdp_v2.audit import VALUATION_REQUIRED_COLUMNS, audit_active
...
def audit_database_as_of(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"qdp_as_of_{cutoff.replace('-', '')}_", dir=str(runtime_root)
    ) as temporary:
        ...
        with open_guarded_duckdb(
            ...
            for domain, dataset_id in sorted(datasets.items()):
                source_manifest = _active_manifest(
                    source_root, dataset_id, domain
                )
        ...
        snapshot_active = dict(active)
        snapshot_active["active_as_of_date"] = cutoff
        snapshot_active["updated_at"] = utc_now()
        snapshot_active["source_active_manifest"] = str(
            (source_root / "active" / "active.json").resolve()
        )
        snapshot_scope = dict(snapshot_active.get("scope", {}) or {})
        snapshot_scope["end_date"] = cutoff
        snapshot_scope["as_of_snapshot"] = True
        snapshot_active["scope"] = snapshot_scope
    ...
    null_cutoff_rows = sum(
        int(profile["null_cutoff_date_row_count"]) for profile in profiles.values()
    )
    forbidden_rows = sum(
        int(profile["forbidden_after_cutoff_row_count_in_audit_snapshot"])
        for profile in profiles.values()
    )
    boundary_excluded = sum(
        int(profile["source_rows_after_cutoff_excluded"])
        for profile in profiles.values()
    )
            ...
            cutoff_findings.append(
                _finding(
                    ...
                    "cutoff_date_is_null",
                    {
                        ...
                        "null_cutoff_date_row_count": profile[
                            "null_cutoff_date_row_count"
                        ],
                    },
                )
            )
    payload["findings"] = [*list(payload.get("findings", []) or []), *cutoff_findings]
    payload["errors"] = [
        item for item in payload["findings"] if item.get("severity") == "high"
    ]
    payload["warnings"] = [
        item for item in payload["findings"] if item.get("severity") == "medium"
    ]
    ...
    payload["as_of"] = {
        ...
        "forbidden_after_cutoff_row_count_in_audit_snapshot": forbidden_rows,
        "null_cutoff_date_row_count": null_cutoff_rows,
        "source_rows_after_cutoff_excluded": boundary_excluded,
...
def _snapshot_manifest_as_of(
    ...
    *,
    ...
    excluded_count = 0
    source_rows_after = 0
    included_date_mins: list[str] = []
    included_date_maxs: list[str] = []
    ...
    for index, entry in enumerate(source_manifest.shards):
        ...
        if start and start > as_of_date:
            excluded_count += 1
            source_rows_after += int(entry.row_count)
            continue
        requires_filter = not end or end > as_of_date
        if not requires_filter:
            ...
            direct_count += 1
            if start:
                included_date_mins.append(start)
            if end:
                included_date_maxs.append(end)
            continue
        ...
        source_rows_after += int(row[1] or 0)
        if not retained_rows:
            excluded_count += 1
            continue
        target = materialized_root / domain / f"shard_{index:04d}.parquet"
        ...
        if actual_rows != retained_rows:
            raise ValueError(
                f"as_of_materialized_row_mismatch:{domain}:{index}:"
                f"{retained_rows}:{actual_rows}"
            )
        filtered_start = str(row[2] or "")
        filtered_end = str(row[3] or "")
        retained.append(
            ShardManifestEntry(
                ...
                metadata={
                    **dict(entry.metadata),
                    ...
                    "source_rows_after_cutoff_excluded": int(row[1] or 0),
                },
            )
        )
        materialized_count += 1
        if filtered_start:
            included_date_mins.append(filtered_start)
        if filtered_end:
            included_date_maxs.append(filtered_end)
    ...
    forbidden = int(boundary[0] or 0)
    null_dates = int(boundary[1] or 0)
    if forbidden:
    ...
    if "unknown_industry_rows" in quality:
        quality["physical_source_unknown_industry_rows"] = quality.pop(
            "unknown_industry_rows"
        )
    use_declared_range = domain not in AS_OF_DATE_COLUMNS
    start_date = str(boundary[2] or "") if use_declared_range else ""
    end_date = str(boundary[3] or "") if use_declared_range else ""
    snapshot = DatasetManifest(
        ...
        end_date=end_date,
        row_count=sum(int(item.row_count) for item in retained),
        shards=retained,
        source=source,
        quality=quality,
    ...
    profile = {
        ...
        "audit_snapshot_min_date": str(boundary[2] or ""),
        "audit_snapshot_max_date": str(boundary[3] or ""),
        ...
        "direct_reference_shard_count": direct_count,
        "materialized_boundary_shard_count": materialized_count,
        "excluded_after_cutoff_shard_count": excluded_count,
    ...
    if args.as_of:
        ...
        if args.write_audit:
            ...
            write_path = (
                root
                / "audits"
                / f"database_audit_as_of_{str(args.as_of).replace('-', '')}.json"
            )
        payload = audit_database_as_of(
            as_of_date=str(args.as_of), write_path=write_path, **common
        )
    else:
