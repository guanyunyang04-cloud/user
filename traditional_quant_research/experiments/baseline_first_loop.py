...
DEFAULT_RESEARCH_LOG = Path("traditional_quant_research/research_log/2026-06-02_first_loop_baseline.md")
...
def run_baseline_first_loop(
    *,
    ...
    summary_markdown = render_summary_markdown(result, ic_summary, quantile_summary)
...
def render_summary_markdown(result: dict[str, Any], ic_summary: pd.DataFrame, quantile_summary: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        f"- Method: z-score baseline factors, average into `baseline_score`, evaluate `{result.get('label')}` with IC, quantile returns, and Top-{result.get('top_n')} equal-weight baseline.",
