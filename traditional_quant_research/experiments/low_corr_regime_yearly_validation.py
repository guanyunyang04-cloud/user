"""Validate low-corr regime filters across multiple evaluation years."""
...
from traditional_quant_research.experiments.low_corr_regime_filter import (
    ...
    parse_regime_filter_specs,
    run_low_corr_regime_filter,
)
...
DEFAULT_BREADTH_THRESHOLDS = (0.40, 0.45, 0.50)
DEFAULT_FINAL_END_DATE = "2026-06-01"


def build_regime_threshold_specs(
    *,
    market_ret_thresholds: Sequence[float] = DEFAULT_MARKET_RET_THRESHOLDS,
    breadth_thresholds: Sequence[float] = DEFAULT_BREADTH_THRESHOLDS,
) -> str:
    """Build semicolon-separated regime filter specs from threshold grids."""

    if not market_ret_thresholds:
        raise ValueError("market_ret_thresholds must not be empty")
    if not breadth_thresholds:
        raise ValueError("breadth_thresholds must not be empty")
    specs = []
    for market_ret in market_ret_thresholds:
        for breadth in breadth_thresholds:
            specs.append(f"market_ret_20d_mean>={float(market_ret):g},breadth_20d_positive_rate>={float(breadth):g}")
    return ";".join(specs)


def year_windows(year: int, *, final_end_date: str | None = DEFAULT_FINAL_END_DATE) -> dict[str, str]:
    """Return prior-year fit and current-year evaluation windows."""
    ...
    prior_year = year - 1
    return {
        "history_start_date": f"{prior_year}-01-01",
        "fit_start_date": f"{prior_year}-01-01",
        "fit_end_date": f"{prior_year}-12-31",
        "start_date": start_date,
        "end_date": end_date,
    }


def run_low_corr_regime_yearly_validation(
    *,
    root: str | None = None,
    ...
    regime_filter_specs: str | None = None,
    market_ret_thresholds: Sequence[float] = DEFAULT_MARKET_RET_THRESHOLDS,
    breadth_thresholds: Sequence[float] = DEFAULT_BREADTH_THRESHOLDS,
    ...
    """Run prior-year-fit regime validations over multiple years."""

    if not years:
        raise ValueError("years must not be empty")
    run_id = f"low_corr_regime_yearly_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    run_dir = output_dir / run_id
    child_output_dir = run_dir / "year_runs"
    run_dir.mkdir(parents=True, exist_ok=True)

    effective_regime_specs = regime_filter_specs or build_regime_threshold_specs(
        market_ret_thresholds=market_ret_thresholds,
        breadth_thresholds=breadth_thresholds,
    )
    regime_filter_configs = parse_regime_filter_specs(effective_regime_specs, include_baseline=include_baseline)
    ...
    for year in years:
        ...
        child_result = run_low_corr_regime_filter(
            root=root,
            ...
            regime_filter_configs=regime_filter_configs,
        ...
        _append_child_frame(summary_frames, child_dir / "regime_summary.csv", eval_year=int(year), run_id=child_result["run_id"])
        _append_child_frame(report_frames, child_dir / "regime_filter_reports.csv", eval_year=int(year), run_id=child_result["run_id"])
        _append_child_frame(period_frames, child_dir / "regime_period_summary.csv", eval_year=int(year), run_id=child_result["run_id"])

    yearly_summary = pd.concat(summary_frames, ignore_index=True) if summary_frames else pd.DataFrame()
    regime_reports = pd.concat(report_frames, ignore_index=True) if report_frames else pd.DataFrame()
    period_summary = pd.concat(period_frames, ignore_index=True) if period_frames else pd.DataFrame()
    aggregate = summarize_yearly_validation(yearly_summary)
    ...
    result = {
        ...
        "regime_filter_specs": effective_regime_specs,
        "config_count": int(len(regime_filter_configs)),
        ...
        "candidate_count": 0,
        "assessment": "multi-year backtest_only regime evidence; no strategy candidate is promoted by this experiment alone",
        "output_dir": str(run_dir),
    }

    yearly_summary.to_csv(run_dir / "yearly_validation_summary.csv", index=False, encoding="utf-8-sig")
    regime_reports.to_csv(run_dir / "yearly_validation_regime_reports.csv", index=False, encoding="utf-8-sig")
    period_summary.to_csv(run_dir / "yearly_validation_period_summary.csv", index=False, encoding="utf-8-sig")
    aggregate.to_csv(run_dir / "yearly_validation_aggregate.csv", index=False, encoding="utf-8-sig")
    (run_dir / "summary.json").write_text(json.dumps(_json_ready(result), ensure_ascii=False, indent=2), encoding="utf-8")
    markdown = render_yearly_validation_markdown(result, aggregate, yearly_summary)
...
def summarize_yearly_validation(yearly_summary: pd.DataFrame) -> pd.DataFrame:
    """Aggregate per-year validation rows by config and protocol."""
...
def render_yearly_validation_markdown(
    ...
    lines = [
        ...
        f"- Regime Specs: `{result.get('regime_filter_specs')}`.",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--regime-filter-specs", default="", help="Semicolon-separated explicit regime filters. Empty uses threshold grid flags.")
    parser.add_argument("--market-ret-thresholds", default=",".join(str(value) for value in DEFAULT_MARKET_RET_THRESHOLDS))
    parser.add_argument("--breadth-thresholds", default=",".join(str(value) for value in DEFAULT_BREADTH_THRESHOLDS))
...
def main() -> None:
    args = parse_args()
    result = run_low_corr_regime_yearly_validation(
        root=args.root,
        ...
        regime_filter_specs=args.regime_filter_specs.strip() or None,
        market_ret_thresholds=_parse_float_tuple(args.market_ret_thresholds),
        breadth_thresholds=_parse_float_tuple(args.breadth_thresholds),
