...
from daily_research.path_policy import v2_horizon_concentration_train_contract_scout as scout
...
def test_build_training_tasks_uses_calibrated_horizon_soft_penalty_contract() -> None:
    tasks = scout.build_training_tasks(output_root=Path("anchor"), seeds=(7,))
    ...
    assert task["loss_profile_contract"]["decision_score_calibration"]["enabled"] is True
    assert "--forecast-memmap-manifest" in task["command"]
    assert task["command"][task["command"].index("--forecast-loss-profile") + 1] == "horizon_30d_soft_penalty_v1"
    assert task["command"][task["command"].index("--forecast-selection-profile") + 1] == "decision_utility"


def test_write_task_list_records_posthoc_source_and_manifest_validation(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(scout, "validate_source_manifest", lambda *_args, **_kwargs: {"status": "ok", "blockers": []})

    path = scout.write_task_list(tmp_path / "out", seeds=(7, 11, 19))
    ...
    assert payload["loss_profile"] == "horizon_30d_soft_penalty_v1"
    assert payload["source_manifest_validation"]["status"] == "ok"
    assert payload["boundary"]["promotion_allowed"] is False
    assert payload["boundary"]["source_posthoc_repair_anchor_run_tag"] == scout.BASELINE_ANCHOR_RUN_TAG
...
def test_run_training_tasks_blocks_when_source_manifest_invalid(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    payload = scout.run_training_tasks(output_root=tmp_path / "out", seeds=(7,))

    assert payload["status"] == "blocked"
    assert "invalid_or_missing_local_state_seed7_manifest" in payload["blockers"]
    assert (tmp_path / "out" / "v2_horizon_concentration_train_contract_training_summary.json").exists()
