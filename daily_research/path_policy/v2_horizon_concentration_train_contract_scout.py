...
RUN_TAG = "mh_v2_horizon_concentration_train_contract_anchor_20260603_01"
RESEARCH_PROGRAM = v2.RESEARCH_PROGRAM
STUDY_FAMILY = "v2_horizon_concentration_train_contract_scout"
DATASET_ID = v2.DATASET_ID
V2_STRICT_POOL_VIEW_ID = v2.V2_STRICT_POOL_VIEW_ID
V2_STATUS_SIDECAR_ID = v2.V2_STATUS_SIDECAR_ID
BASELINE_ANCHOR_RUN_TAG = "mh_v2_horizon_concentration_repair_anchor_20260603_01"
SOURCE_LOSS_ANCHOR_RUN_TAG = "mh_v2_local_state_loss_calibration_anchor_20260603_01"
...
def _study_tag(seed: int) -> str:
    return f"mh_v2_horizon_30d_soft_penalty_seed{int(seed)}_20260603_01"
...
def _loss_contract() -> dict[str, Any]:
    ...
    calibration = dict(contract.get("decision_score_calibration", {}) or {})
    if not bool(calibration.get("enabled", False)):
        raise ValueError(f"loss_profile_missing_decision_score_calibration: {LOSS_PROFILE}")
    return contract
        ...
        tasks.append(
            {
                ...
                "source_loss_anchor_run_tag": SOURCE_LOSS_ANCHOR_RUN_TAG,
                "source_posthoc_repair_anchor_run_tag": BASELINE_ANCHOR_RUN_TAG,
...
def write_task_list(
    ...
    *,
    ...
    root = _anchor_root(output_root)
    resolved_seeds = _parse_seeds(seeds)
    path = root / "v2_horizon_concentration_train_contract_task_list.json"
    payload = {
        "schema_version": 1,
        "run_tag": RUN_TAG,
        "research_program": RESEARCH_PROGRAM,
...
def run_training_tasks(
    *,
    ...
    if source_validation["status"] != "ok":
        ...
        _write_json(root / "v2_horizon_concentration_train_contract_training_summary.json", payload)
        return payload
    resolved_seeds = _parse_seeds(seeds)
    completed: list[str] = []
    failed: list[str] = []
...
def run_comparison_and_audits(
    *,
    ...
    _write_json(root / "v2_horizon_concentration_train_contract_summary.json", summary)
    return summary


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run daily_research v2 train-side horizon concentration contract scouts.")
    ...
    if _active_artifact_has_diff():
        raise ValueError(f"active_artifact_diff_blocker: {ACTIVE_MANIFEST} has uncommitted diff.")
    seeds = CONFIRM_SEEDS if args.confirm_seeds else _parse_seeds(args.seeds)
