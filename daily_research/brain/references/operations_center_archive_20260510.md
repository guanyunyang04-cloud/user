# operations_center.md 强归档快照 2026-05-10

> 本文件由主分脑强归档维护生成，保存归档前完整内容。当前入口以 `daily_research/brain/` 下同名主文件为准。
...
## 2026-05-10 r52 native source-delta closure 操作结果
- 已完成代码修复：r52/r51 native projection 的 source 硬门改为持仓 delta 口径，旧 source mask 仅作为 prior / diagnostics；simulator 在 native target 有效时由 `target_weight - current_weight` 直接推导 source / receiver target；pipeline 与 study progress 增加 native-to-simulator alignment 证据字段。
- 新增重点诊断：`native_source_delta_alignment_support`、`native_negative_delta_count`、`native_source_candidate_count`、`native_source_target_count`、`native_source_blocked_by_legacy_mask`、`native_source_audit_threshold_gap`、`native_target_valid`、`native_target_constraint_violations`。
- 验证命令已通过：`py_compile` 覆盖 `model_seq_v3.py`、`portfolio_simulator.py`、`run_self_optimizing_study.py`、`pipeline_utils.py` 与合同测试文件；完整 `daily_research.continuous_policy.tests.test_portfolio_daily_strategy_contracts` 为 `90` 项 OK；`git diff --check` 通过。
- dry-run 已通过：`self_opt_study_r52_native_source_delta_closure_dryrun_20260510_02`。必须读取 `study_plan.json` / `study_progress.json` 中的 `day_set_native_allocation_vector_support = true`、`native_source_delta_alignment_support = true`、`full_universe_train_solver_effective = false`、`confirmatory_enabled = false`。
- safe screening 已完成：`self_opt_study_r52_native_source_delta_closure_screening_safe_20260510_02`。产物入口为 `daily_research/output/continuous_policy/studies/self_opt_study_r52_native_source_delta_closure_screening_safe_20260510_02/study_summary.json`、`trial_ranking.csv`，以及 `daily_research/output/continuous_policy/protocols/self_opt_study_r52_native_source_delta_closure_screening_safe_20260510_02__trial_*/protocol_summary.json` 和对应 `models/...__trial_*__train/training_diagnostics.json`。
- 筛选结果：`completed_trial_count = 3`、`failed_trial_count = 0`、`confirmatory_completed_trial_count = 0`；resource gate 在筛选末尾触发，失败项为 `source_release_dead`、`receiver_deploy_not_clean`、`economic_signal_too_weak`。trial_01 / trial_02 的 source 通道恢复，分别为 `source_target_count = 51 / 260`、`source_realized_sell_rate = 1.0 / 1.0`；trial_03 仍为 `source_target_count = 0`、`source_realized_sell_rate = 0`。
- 操作判定：r52 native source-delta closure 不进入 confirmatory；不得把本轮写成策略有效。后续操作优先级是修 native target valid rate、source delta 阈值和 turnover 约束，而不是直接加 epoch 或恢复 r50 true solver。
