...
### 5.2 执行端的瓶颈是最终升级判决还没做完
- 当前默认主线并不是被新方案全面推翻，而是已经遇到两组值得升级的 shortlist。
- 真正的短期瓶颈不是继续盲扫参数，而是把这两组候选的 head-to-head 做完，形成清晰切换或不切换结论。
...
## 7. 接下来最值得投入的研究方向
### 7.1 第一优先级：做完执行端最终升级判决
- 当前收益最高的投入，不是再开一堆候选，而是完成 shortlist 的最终比较。
- 只有这一步做完，执行端才会真正从“已有稳定主线”走向“下一版正式主线”。
