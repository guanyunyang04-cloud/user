...
- r41-r47 已完成代码入口或 dry-run，但均为保留检查点，不是当前默认长训入口；r48 formal screening + confirmatory 已完成且 stable confirm 为空；r49 / r50 / r51 仍是 research profile，不是 production 默认。r51 当前是轻量 native allocation vector 主线入口，当前有效证据基线仍是 r39。
