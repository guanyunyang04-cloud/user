...
## 2026-05-07 r47 true convex solver allocation 操作入口
- r47 profile / loss：`split_heads_portfolio_daily_true_convex_solver_allocation_r47` / `alpha_result_value_budget_split_v32`。
- r47 objective / budget：`end_to_end_allocation_layer_v1`、`allocation_layer_v1`、`end_to_end_allocation_layer_v1`。
- r47 默认 screening 资源口径：profile 内 `epochs = 10`、`min_epochs = 7`；命令行未显式传入 `--epochs` / `--min-epochs` 时不得覆盖该默认值。
- r47 依赖检查命令：`C:/Users/ASUS/miniconda3/envs/yolos/python.exe -m pip show cvxpy cvxpylayers diffcp scs clarabel`。
- r47 诊断读取：训练 diagnostics 需优先读取 `portfolio_cvxpy_convex_layer_status` 与 `portfolio_cvxpy_convex_allocation_terms`，其中 `solver_success_rate`、`solver_regret`、`solution_tracking_loss`、`gross_residual`、`turnover_residual`、`position_residual`、`unsupported_mass`、`false_source_mass`、`cash_timing_loss`、`path_risk_loss` 与 `total` 是判断是否值得正式 screening 的关键前置信号。
- r47 dry-run 命令：`$env:KMP_DUPLICATE_LIB_OK='TRUE'; $env:PYTHONUTF8='1'; $env:PYTHONIOENCODING='utf-8'; C:/Users/ASUS/miniconda3/envs/yolos/python.exe -m daily_research.continuous_policy.run_self_optimizing_study --search-profile split_heads_portfolio_daily_true_convex_solver_allocation_r47 --objective-profile end_to_end_allocation_layer_v1 --budget-semantics allocation_layer_v1 --budget-calibration end_to_end_allocation_layer_v1 --budget-objective result_value_v10 --study-tag <tag> --dry-run`。
