...
- v2 high-return model discovery 编排已实现并完成 smoke32 pipeline 与 full-pool augmented pilot 验证：`mh_v2_hrd_same_pilot_pilot_gru_seed7_20260604_01` 为 `pilot_only`，augmented feature/forecast pipeline 已跑通，quick bridge 前半生成 score panel 但 shared backtest 因仅 1 个有效交易日失败；这不是模型质量证据，下一步是足够日期覆盖的 full augmented single-seed multi-split scout。
...
- P3b：数据扩充和高收益发现路线下一步是足够测试日期覆盖的 full augmented single-seed multi-split scout / quick bridge leaderboard；tiny pilot 只能验证 throughput，只有 shortlist finalist 才跑 seeds `7,11,19`。smoke、pilot、single-seed、timeout 不得写成 completed model-quality evidence。
