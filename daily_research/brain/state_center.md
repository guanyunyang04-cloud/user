...
- v2 high-return model discovery 已完成 long-history augmented / 主板宽池 GRU seed7 multi-split scout、same-period control，以及首轮 objective/loss scout。long-history 与 proxy-loss 结果都只在 split `a` 正 transfer：最佳 split-a small-capital variant excess Sharpe `1.5369`、excess annual return `0.2669`，但 `b/c/long` 和 same-period control 负 transfer；`topn_excess_rank_v1`、`score_to_weight_proxy_v1`、`bad_month_aware_v1` 也未达到两个正 transfer split。因此不是 finalist，不允许三 seed、promotion、paper/live 或 active change。下一步转 split/regime/label/root-cause 诊断，而不是堆更大模型或调执行壳。
...
- P3b：数据扩充和高收益发现路线已完成 long-history augmented GRU seed7 scout / quick bridge / `long_a` small-capital matrix / 首轮 objective-loss scout；当前结论是单 split 可调强但跨 split / same-period 不稳。下一步优先 split `a` vs `b/c/long` 的 regime、volatility、reversal、liquidity、label/selection alignment 与 datecap sensitivity 诊断；只有至少两个 split / pool view 显示正 transfer 的 shortlist finalist 才跑 seeds `7,11,19`。smoke、pilot、single-seed、timeout 不得写成 completed model-quality evidence。
