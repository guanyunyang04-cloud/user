...
`current_evidence`: `daily_only_summary_v2` remains the default research mainline because it keeps validation IC near the old default, improves test IC to `0.1677`, improves test Top1 to `15.65%`, and simplifies the input surface to 32 daily features; old `all_channels_base_summary` remains the broad-TopK baseline because its test Top3/Top10 `11.85%/10.24%` exceed the new default `8.89%/7.00%`; `daily_only_summary_v2_price_delta` completed with close log-delta rhythm supervision and improved validation/test IC to `0.1605/0.1781`, but test Top1 PV alpha was `15.41%`, not better than default; `daily_only_summary_v2_ohlcva_aux_low` completed with low VA weights `0.02/0.01`, validation/test IC `0.1626/0.1755`, and strong test Top1/Top3 PV alpha `23.62%/13.31%`, so it is still the best balanced narrow-selection candidate but not auto-promoted; `daily_only_summary_v2_ohlcva_path_equal` completed with OHLCVA six fields equally entering main path_loss, validation/test IC `0.1497/0.1434`, test Top1/Top3/Top10 `26.20%/11.65%/6.34%`, giving the strongest isolated Top1 but worse IC and broader TopK than low VA auxiliary; high-weight `daily_only_summary_v2_ohlcva_aux` had validation/test IC `0.1502/0.1459` and test Top1/Top3/Top10 `22.06%/12.76%/8.13%`, showing narrow TopK help but IC drag; low VA plus price_delta underperformed both single additions with validation/test IC `0.1525/0.1527`; summary_v2 multi-horizon OHLC with all channels improved validation and test Top1 but lowered test IC and broad TopK; `summary_v2_no60` is the strict control that removes only the duplicated 60d OHLC summary window, improving test Top1/Top3/Top10 versus all-channel summary_v2 to `16.58%/12.98%/9.47%` but not chosen as default because IC is weaker; daily-only no-minute improved test IC and Top1/Top3 but weakened test Top10; `no_intraday_summary` achieved test IC `0.1745` but weak Top10 `5.68%`, while `no_limit_structure` test IC was `0.1482` with weak Top10 `5.61%`, so minute-derived inputs are not clearly necessary for IC or narrow selection but may interact for wider TopK breadth; direct-value 5d/10d/60d rankers remain weaker than the default 60d OHLC path-output mainline; all path-value spread remains research evidence, not promotion evidence.
