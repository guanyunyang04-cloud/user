...
- `alpha_multi_horizon_utility_policy_v1` 的当前 blocker 是 corrected mainboard baseline 只达到 `near_pass` 且旧 `156` feature schema / short_v5b payload 不可 replay：Stage 2.8 历史结论仍作为 brain-confirmed evidence 保留；旧 wrong-universe rebuild anchor 只能诊断。下一步应围绕 hit-lift negative seed 拆解、feature schema recovery、BaoStock-first lineage 差异、target calibration / hit-lift-preserving regularization 做窄矩阵；不得从 corrected near-pass 直接 promotion、执行解冻或 broad Stage 3 expansion。
