...
- P3c：QDP alpha_v2 数据线下一步先做 `style_structural_v1` 与 `style_structural_alpha_v2_label_v2` 的同配置 research-only 对照；alpha_v2 consumption smoke 只能证明 daily protocol 可读取新 pack、feature schema、static context、label v2 和 role split，不能用于模型质量或 promotion 判断。
