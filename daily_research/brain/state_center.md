...
`current_evidence`: today-close path-only is the default research mainline; summary_v2 multi-horizon OHLC improved validation and test Top1 but lowered test IC and broad TopK, so it remains comparison evidence; all path-value spread remains research evidence, not promotion evidence.
