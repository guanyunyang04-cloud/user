...
- P3c：QDP alpha_v2 强模型路线当前以 `path_policy_alpha_v2_time_efficient_topk_training_contract_plan_20260618.md` 为热路径计划，旧 `path_policy_alpha_v2_strong_model_research_plan_20260617.md` 降为历史计划/背景。`personal_time_efficient_topk_v1` 训练合同、`validation_loss` best 选择、真实 alpha_v2 pack smoke、Stage 0 old-contract validation-loss audit、首个 h256 seed7 full run 均已完成；新合同首个 full run 未打赢旧合同 anchor，不进入 multi-seed、bridge、candidate matrix 或 execution review。
- P3c-now：Stage 1 loss-only scouting 已暂停；`decision_score_topk_alignment_v1`、`score_monthly_robust_v1`、`horizon_30d_soft_penalty_v1` 均作为旧合同证据收口。当前优先级是分析 `personal_time_efficient_topk_v1` 首轮 full run 为什么 epoch1 后 validation loss 恶化、且同候选 personal topK 弱于 anchor；下一步优先做受控诊断或更轻量对照，例如 component validation-loss attribution、h192/更强正则、降低 `time_eff_topk_alignment` / `decision_rank_aux` 权重。当前仍禁止 multi-seed、bridge、candidate matrix、execution-candidate 或 active artifact 变更。
