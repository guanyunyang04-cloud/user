...
- P4：中期正路是把 r46 的 torch 图内 differentiable convex allocation surrogate 继续向正式可微/凸优化 allocation layer 迁移；`solve_semidifferentiable_allocation` 仍保留为推理/模拟安全层，simulator guard 只保留最后安全裁剪。
