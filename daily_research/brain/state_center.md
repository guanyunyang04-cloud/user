...
- v2 high-return model discovery 编排已实现并完成 smoke32 pipeline、full-pool augmented pilot、same-period augmented datecap2 scout：`mh_v2_hrd_same_same_gru_seed7_20260604_01` 为 `scout_only`，覆盖 train `933` 日、validation/test 各 `211` 日且 quick bridge 已完成；但 forecast verdict 为 `forecast_failed`，excess Sharpe `-0.5999`、positive month ratio `0.5455`、worst monthly excess return `-0.1936`，结论是 date-wise scout/bridge 路径已修通但该 GRU same-period candidate 偏弱，不是模型质量证据。下一步是 long-history / multi-split single-seed scout，而不是三 seed 确认或执行解冻。
...
- P3b：数据扩充和高收益发现路线下一步是 long-history augmented single-seed multi-split scout / quick bridge leaderboard；same-period datecap2 GRU scout 已证明路径修通但候选偏弱。只有至少两个 split 显示有 forecast-to-portfolio transfer 的 shortlist finalist 才跑 seeds `7,11,19`。smoke、pilot、single-seed、timeout 不得写成 completed model-quality evidence。
