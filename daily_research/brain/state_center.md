...
- r48 未过 stable confirm，不得 promotion、不得 live、不得改 active artifact。
...
- P3a：r48 formal screening + confirmatory 已完成且失败，不能继续用同一 tag 重跑期待随机改善。任何后续 r49+ 或 r48 修正版都必须先明确要解决 `source_target_count = 0`、`exposure_utilization` 低、training evidence edge、cash timing gate 与 stable confirm 失败，而不是回退 r47/r46/r45/r44/r43/r42/r41 长训。任何 r41+ 正式研究都必须先完成无冲突进程、无同名 tag、dry-run / contract gate 预检，并以前台持久日志和进度文件方式运行；代码测试通过不能写成策略有效。
