...
- `alpha_multi_horizon_utility_policy_v1` 的当前 blocker 已从 target/loss stability 转为主板 universe 口径修正后的 rebuild baseline：Stage 2.8 历史结论仍作为 brain-confirmed evidence 保留；旧 new-lineage rebuild anchor 现在因包含创业板/科创板而降级为 `wrong_universe_diagnostic`。下一步优先用 `policy_pool_view__74f45f4f83263bccd64a8027` 重跑 mainboard-only baseline，再判断 seed11 负月份、target definition、regime split、feature scaling/interaction ablation、hit-lift-preserving rank regularization 或 horizon concentration/utility tradeoff；不得从旧错误 universe anchor 直接 promotion 或 broad Stage 3 expansion。
...
- PathPolicy / TDX-free 数据平台：`daily_research/brain/references/path_policy_execution_issue_learning_20260523.md`、`daily_research/brain/references/tdx_free_data_platform_decision_20260523.md`、`daily_research/brain/references/tdx_free_data_platform_v2_20260523.md`；主板池口径修正：`daily_research/brain/references/alpha_multi_horizon_mainboard_pool_correction_20260601.md`。
