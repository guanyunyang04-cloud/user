...
- P3c：QDP alpha_v2 强模型路线以 `path_policy_alpha_v2_strong_model_research_plan_20260617.md` 为当前执行计划。下一步先在 h256 固定架构上做 loss/output alignment，必要时新增窄 loss 合同和测试；再做 fusion/head/GRU/Transformer/DLinear/router 的一变量 hybrid ablation；速度问题只按真实吞吐瓶颈优化。multi-seed、score-backtest bridge、candidate matrix、execution-candidate 仍后置，不得从 single-seed 或 throughput run 推导正式模型质量或执行结论。
- P3c-now：Stage 1 loss-only scouting 已暂停；`decision_score_topk_alignment_v1`、`score_monthly_robust_v1`、`horizon_30d_soft_penalty_v1` 均作为非 anchor-beating 结果收口。下一步不要继续随机换 loss，应转入 Stage 2 hybrid architecture/head/router ablation，或先做 h30soft 的 checkpoint/output diagnostics，解释“forecast/test-only 强但 validation-selected same-candidate 弱”的断点。当前仍禁止 multi-seed、bridge、candidate matrix、execution-candidate 或 active artifact 变更。
