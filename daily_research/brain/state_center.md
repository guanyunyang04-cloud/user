...
- `daily_research_v2_research_reset` 是当前 path_policy / multi-horizon 新主线：不再追求严格复刻旧 Stage 2.8 / short_v5b，改为在 BaoStock-first corrected mainboard 数据湖上重新建设 feature/input、模型与 gate。当前直接 blocker 是 corrected mainboard baseline 只达到 `near_pass`，尤其 hit lift min `-0.007156`；旧 `156` feature schema 与 short_v5b payload 不再阻塞 v2 主线，但保留为历史参考和可选旁路审计。下一步应围绕 v2 feature-input audit、Amount 单位与 missing/fill 对新特征质量的影响、hit-lift repair、target calibration / hit-lift-preserving regularization 做窄矩阵；不得从 corrected near-pass 直接 promotion、执行解冻或 broad Stage 3 expansion。
...
- P3：`daily_research_v2_research_reset` 已接管 path_policy / multi-horizon 研究方向；旧 `alpha_multi_horizon_utility_policy_v1`、Stage 2.8、Stage 3A-3G 与 short_v5b 作为历史参考和方法库保留，不再作为必须复刻的当前目标。v2 当前基线是 corrected mainboard-only `near_pass` anchor，下一步做 v2 feature-input audit、hit lift repair、target calibration / hit-lift-preserving regularization 窄矩阵。cap80 和旧 replay 差异结果只能作为诊断线索，不能作为 v2 gate evidence。
