...
- r41-r46 是当前 end-to-end allocation layer 升级链：r41 risk-sensitive、r42 utility-credit、r43 primal-dual decision、r44 entropic transport、r45 conservative transport、r46 differentiable convex allocation。事实：r46 已新增 `alpha_result_value_budget_split_v31`、`_portfolio_differentiable_convex_allocation_loss`、torch 图内 KKT / constraint residual / cash timing / support violation loss、日级 gross/turnover/position/cash timing target 接线、behavior-support / conservative OPE、路径级 CVaR / drawdown / OCE 风险项、component diagnostics、`split_heads_portfolio_daily_differentiable_convex_allocation_r46` 与 resource gate；推断：r46 是当前下一优先 research 入口，r41-r45 仅作为保留检查点，均不得替代 r39 证据基线。
...
- 当前 r33 profile 标记：`split_heads_portfolio_daily_source_forward_proxy_r33`；当前 r34 profile 标记：`split_heads_portfolio_daily_allocation_breadth_r34`；当前 r35 profile 标记：`split_heads_portfolio_daily_unified_allocation_r35`；当前 r36 profile 标记：`split_heads_portfolio_daily_risk_aware_unified_allocation_r36`；当前 r37 profile 标记：`split_heads_portfolio_daily_decision_focused_allocation_r37`；当前 r38 profile 标记：`split_heads_portfolio_daily_source_hard_negative_regret_r38`；当前 r39 profile 标记：`split_heads_portfolio_daily_allocation_objective_consolidation_r39`；当前 r41-r46 profile 分别为 risk-sensitive、utility-credit、primal-dual decision、entropic transport、conservative transport 与 differentiable convex allocation。
- 当前有效证据基线仍为 r39 的 `alpha_result_value_budget_split_v25` 与 `portfolio_daily_ranking_v2_gated`；r46 新实现标记为 `alpha_result_value_budget_split_v31`、`portfolio_differentiable_convex_allocation_total`、`supports_portfolio_differentiable_convex_allocation_loss`、`supports_portfolio_differentiable_convex_allocation_diagnostics`、`supports_portfolio_path_risk_loss`、`end_to_end_allocation_layer_v1`、`allocation_layer_v1` 与 `end_to_end_allocation_layer_v1`，但尚未运行完整 study。r41-r45 目标和 loss 作为保留检查点，不能替代 r46 当前 research 入口或 r39 证据基线。
...
- P3a：当前只把 r46 作为下一 research 入口；先跑 r46 短 screening + resource gate，不再优先启动 r45/r44/r43/r42/r41 长训。任何 r41+ 正式研究都必须先完成无冲突进程、无同名 tag、dry-run / contract gate 预检，并以前台持久日志和进度文件方式运行；代码测试通过不能写成策略有效。
- P4：中期正路是把 r46 的 torch 图内 differentiable convex allocation surrogate 继续向正式可微/凸优化 allocation layer 迁移；当前 r46 已把分项诊断、日级约束 target、behavior support / OPE 与路径风险写入训练图，但仍不是完整 cvxpylayers / convex solver 依赖层。`solve_semidifferentiable_allocation` 仍保留为推理/模拟安全层，simulator guard 只保留最后安全裁剪。
