...
- `alpha_multi_horizon_utility_policy_v1` 的当前 blocker 是 corrected mainboard baseline 只达到 `near_pass`，旧 `156` feature schema 未恢复，short_v5b payload 不可 replay，且 TQ/BaoStock 审计虽显示 next-open labels 基本等价，但 OHLCV 仍有 missing/Amount 口径差异：Stage 2.8 历史结论仍作为 brain-confirmed evidence 保留；旧 wrong-universe rebuild anchor 只能诊断。下一步应围绕旧 manifest/156 schema recovery、Amount 单位和 missing/fill policy diff、hit-lift negative seed、target calibration / hit-lift-preserving regularization 做窄矩阵；不得从 corrected near-pass 直接 promotion、执行解冻或 broad Stage 3 expansion。
...
- P3：`alpha_multi_horizon_utility_policy_v1` 已完成 Stage 2.8 full rolling_liquid500 target/loss 复验、Stage 3A-3C low-seed architecture/input scout、Stage 3D-3F sector input repair scout、Stage 3G sector-relative/regime input scout、2026-06-01 wrong-universe rebuild audit 与 corrected mainboard-only baseline。当前不升级 patch/stock/slot/sector/sector-relative-regime 架构或输入；corrected baseline 为 `near_pass`，下一步才做 hit lift / feature schema / lineage bridge 窄矩阵。cap80 结果只能作为诊断线索，不能作为 gate evidence。
