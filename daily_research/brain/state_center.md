...
`comparison_surface`: `table_path60_baseline`、`path_only_next_open`、`rank_heavy_top1`、`summary_v2_multi_horizon_ohlc`、`summary_v2_no60`、`daily_only_no_minute`、`no_intraday_summary`、`no_limit_structure`、`direct_value_rank_5d`、`direct_value_rank_10d`、`direct_value_rank_60d`。
`archived_or_paused_surface`: `alpha_v2`、`path20`、`symbol_embedding`、`residual_score`、`richer_target`、`ohlcva_unified`。
`current_evidence`: today-close path-only is the default research mainline; summary_v2 multi-horizon OHLC improved validation and test Top1 but lowered test IC and broad TopK, so it remains comparison evidence; `summary_v2_no60` is the strict control that removes only the duplicated 60d OHLC summary window, improving test Top1/Top3/Top10 versus summary_v2 to `16.58%/12.98%/9.47%` but still not replacing the default; current summary_v2 CLIs default to `early_stopping_patience=2` and use vectorized multi-horizon summary loss with unchanged objective; daily-only no-minute improved test IC and Top1/Top3 but weakened test Top10; `no_intraday_summary` achieved test IC `0.1745` but weak Top10 `5.68%`, while `no_limit_structure` test IC was `0.1482` with weak Top10 `5.61%`, so minute-derived inputs are not clearly necessary for IC or narrow selection but may interact for wider TopK breadth; direct-value 5d/10d/60d rankers completed as score-only comparison profiles, with 10d stronger than 5d and 60d showing positive test IC `0.1025` plus Top1/Top3/Top10 PV alpha `9.46%/7.94%/7.17%`, but still weaker than the default 60d OHLC path-output mainline, so they remain comparison evidence; all path-value spread remains research evidence, not promotion evidence.
