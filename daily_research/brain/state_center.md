...
- v2 high-return model discovery 已完成 long-history augmented / 主板宽池 GRU seed7 multi-split scout 与 same-period control：long-history 4 split 仅 `long_a` 为正 transfer，`long_a` small-capital matrix 为 `9/27` research-grade variants，最佳 `h10_mw080_rb20d_all_c10_5_10_regime_off` excess Sharpe `1.5369`、excess annual return `0.2669`；但 same-period control 为负 transfer（excess Sharpe `-0.6746`），因此不是 finalist，不允许三 seed、promotion、paper/live 或 active change。下一步转 objective/label/loss scout，而不是堆更大模型。
...
- P3b：数据扩充和高收益发现路线已完成 long-history augmented GRU seed7 scout / quick bridge / `long_a` small-capital matrix；当前结论是单 split 可调强但跨 split / same-period 不稳。下一步优先 top-decile/topN return objective、score-to-weight proxy loss、bad-month-aware auxiliary loss、local-volatility/reversal penalty 与 high-return label variants。只有至少两个 split / pool view 显示正 transfer 的 shortlist finalist 才跑 seeds `7,11,19`。smoke、pilot、single-seed、timeout 不得写成 completed model-quality evidence。
