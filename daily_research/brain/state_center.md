...
## 2026-04-29 r33 source forward proxy / release conviction 当前状态
- 运行纪律继续作为铁律：所有 `daily_research` 训练、评估、审计与文档检查均须使用 `C:/Users/ASUS/miniconda3/envs/yolos/python.exe` 前台运行，窗口时限按 `10h` 处理；训练/评估结果必须从 diagnostics 核验 `device = cuda`、`cuda_available = true`、`python_executable` 指向 yolos，且任务自然结束后检查残留进程。
- r33 已新增可训练的 `portfolio_daily_source_forward_proxy_keep_risk` 头，并让 label、model、simulator、feedback、continuity metrics、study scoring 与 behavior audit 共同读取该字段；旧 artifact 只允许使用 fallback proxy，避免随机新 head 污染 replay。
- `cp_v3_portfolio_daily_source_forward_proxy_r33_trainable_proxy_bounded_20260429` 证明 training evidence 可达 `sufficient`，但 source 退化为 dormant：`source_target_count = 0`、`receiver_target_count = 1`。该结果说明 proxy 防错方向有效，但不能单独恢复主动卖出。
- `cp_v3_portfolio_daily_source_forward_proxy_r33_source_soft_bounded_20260429` 通过 source-aware soft target 恢复了资金释放链路：confirm 中 `receiver_target_count = 5`、`source_target_count = 3`、`source_realized_sell_rate = 1.0`、`receiver_unrealized_deploy_share = 0.0`；但出现强势误卖，`receiver_minus_source_forward_excess_5d = -0.059150`、`source_forward_excess_5d = +0.189710`、`source_positive_forward_sell_share = 0.6667`、`source_strong_positive_forward_sell_count = 2`。该路线只能作为反例证据。
- `cp_v3_portfolio_daily_source_forward_proxy_r33_release_conviction_bounded_20260429` 把收益和执行恢复得更强，confirm 中 `annual_return = 1.439900`、`sharpe = 3.022400`、`receiver_target_count = 8`、`source_target_count = 6`、`source_realized_sell_rate = 1.0`、`receiver_unrealized_deploy_share = 0.0`，但仍放过正 forward source：`source_positive_forward_sell_share = 0.6667`、`source_strong_positive_forward_sell_count = 2`、`source_max_forward_excess_5d = 0.140600`。结论是 release conviction 只是更强的候选质量信号，不足以单独约束尾部分布。
- 已新增 `portfolio_daily_source_distribution_clean_pass`，把 source 分布清洁从后验报告推进到 simulator/label 层：source candidate 现在必须同时通过 forward proxy、release conviction 与 distribution clean pass；repeat release 只保留极窄的 clean-pass relief，不再靠粗暴放宽 source count。
- `cp_v3_portfolio_daily_source_forward_proxy_r33_release_conviction_cleanpass_repeat_relief_replay_20260429` 证明 clean-pass 方向正确：full replay 中 `source_target_count = 1`、`source_realized_sell_rate = 1.0`、`source_forward_excess_5d = -0.040850`、`source_positive_forward_sell_share = 0.0`、`source_strong_positive_forward_sell_count = 0`、`receiver_minus_source_forward_excess_5d = +0.039088`，且 receiver deploy 语义保持干净。
- `cp_v3_portfolio_daily_source_forward_proxy_r33_cleanpass_bounded_20260429` 的 fresh bounded confirm 仍未达标：`training_evidence_status = insufficient`、`receiver_target_count = 2`、`source_target_count = 1`、`source_realized_sell_rate = 1.0`、`receiver_unrealized_deploy_share = 0.0`、`source_positive_forward_sell_share = 0.0`、`source_strong_positive_forward_sell_count = 0.0`，但 `receiver_minus_source_forward_excess_5d = -0.018510` 且 `failed_stability_checks` 包含 `confirm_source_count_floor` 与 `confirm_receiver_source_forward_spread_floor`。
- 当前本质判断：r33 已经把“不要强势误卖”压进语义层，但代价是 source/receiver 广度不足；瓶颈从“会不会卖错”转为“能否在 clean source 约束下恢复足够多的正确卖出，并同步找到正 forward receiver”。这不是脚本中止问题，也不是单纯 GPU/训练环境问题，而是资金日分配目标仍未被模型稳定学会。
- 当前状态保持 `research / shadow_only`；不得 promotion、不得 live、不得改 active artifact。下一步优先级是：扩大 clean source breadth、恢复 receiver breadth、让 monthly return / exposure utilization / realized deploy / positive spread distribution 继续内生进 objective，最终回到真正的 source/receiver/cash listwise allocation teacher。
