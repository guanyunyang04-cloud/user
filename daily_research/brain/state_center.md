...
- 2026-06-01 TQ vs BaoStock lineage audit：`tq_baostock_lineage_audit_20260601_01` 已实现并运行；旧 `156` feature recovery 在 `daily_research/output`、`cache`、`archive`、brain references、`H:\new_tdx64\PYPlugins\user\t0_project` 和 git manifest history 中未找到完整 156 列，状态仍 `legacy156_not_recovered`。TQ 只读 adapter 已接到旧 `tqcenter.py`，但本机运行返回 `TQ数据接口初始化失败`，因此 OHLCV diff 与 next-open label diff 均为 `blocked_tq_unavailable`，不能证明 TQ 与 BaoStock 等价或不等价；old Stage 2.8 仍为 `old_stage28_still_text_only`。完整证据见 `daily_research/brain/references/alpha_multi_horizon_tq_baostock_lineage_audit_20260601.md`。
...
- `alpha_multi_horizon_utility_policy_v1` 的当前 blocker 是 corrected mainboard baseline 只达到 `near_pass`，旧 `156` feature schema 未恢复，short_v5b payload 不可 replay，且 TQ vs BaoStock OHLCV/label diff 被 TQ 初始化失败阻断：Stage 2.8 历史结论仍作为 brain-confirmed evidence 保留；旧 wrong-universe rebuild anchor 只能诊断。下一步应先修 TQ read-only 初始化或找到旧 manifest，再围绕 hit-lift negative seed、feature schema recovery、BaoStock-first lineage 差异、target calibration / hit-lift-preserving regularization 做窄矩阵；不得从 corrected near-pass 直接 promotion、执行解冻或 broad Stage 3 expansion。
