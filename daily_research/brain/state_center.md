...
- v2 high-return model discovery 已完成 long-history augmented / 主板宽池 GRU seed7 datecap2 scout、same-period control、首轮 objective/loss datecap2 scout，以及 `topn_excess_rank_v1` 正常样本 long-history `a/b/long` scout、same-period control 和 long split small-capital matrix。datecap2 已降级为 `thin_sample_smoke_only`：只证明链路和报告可用，不作为模型方向淘汰依据。正常样本 `topn_excess_rank_v1` 现在满足 finalist 三 seed 前置条件：long-history `a` 与 `long` 正 transfer、same-period 正 transfer，long split small-capital matrix `15/27` research-grade，最佳变体 `h30_mw080_rb10d_all_c10_5_10_regime_off` 超额年化 `0.4284`、超额 Sharpe `1.7463`、月胜率 `0.7273`、最差月 `-0.0776`。仍禁止 promotion、paper/live、broker 或 active change。
...
- P3b：数据扩充和高收益发现路线当前优先 `topn_excess_rank_v1` 正常样本 finalist 三 seed确认。下一步跑 seeds `7,11,19`，尽量复用已有 seed7，只训练缺失 seeds；通过标准是至少两个 seed 正 excess return / excess Sharpe，并在显式 seed aggregation 或 ensemble bridge 证据存在后再谈 ensemble 优于单 seed median。若 seed 稳定性失败，先诊断 split `b`、月收益集中和 seed sensitivity，再试 `score_to_weight_proxy_v1`、`bad_month_aware_v1` 或执行适配。smoke、pilot、datecap2、single-seed、timeout 不得写成 promotion 或 live evidence。
