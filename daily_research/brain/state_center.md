...
- v2 high-return model discovery 已完成 long-history augmented / 主板宽池 GRU seed7 datecap2 scout、same-period control，以及首轮 objective/loss datecap2 scout。datecap2 已降级为 `thin_sample_smoke_only`：只证明链路和报告可用，不作为模型方向淘汰依据。下一步改为正常样本 single-seed objective trials，先逐个 objective 跑关键 split，再根据正常样本 transfer 决定是否扩 split / same-period / 三 seed；仍不允许 promotion、paper/live 或 active change。
...
- P3b：数据扩充和高收益发现路线已完成 datecap2 high-return 链路 smoke；当前修正结论是 datecap2 样本太薄，不能用于淘汰 objective。下一步优先正常样本 single-seed：先跑 `topn_excess_rank_v1` 的 `a/b/long`，再逐个试 `score_to_weight_proxy_v1`、`bad_month_aware_v1` 或其它 label/loss。只有至少两个 split / pool view 在正常样本下显示正 transfer 的 shortlist finalist 才跑 seeds `7,11,19`。smoke、pilot、datecap2、single-seed、timeout 不得写成 completed model-quality evidence。
