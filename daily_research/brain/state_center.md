...
- P3c：QDP alpha_v2 数据线已完成首轮 `style_structural_v1` vs `style_structural_alpha_v2_label_v2` 同口径 research-only 对照。alpha_v2 h256 seed7 默认 best 的 personal top-K validation 选择和同候选 test 均强于旧 v1 默认 h128/h192/h256，对当前“小资金强模型优先”目标匹配较好。下一步优先继续 alpha_v2 强模型研究：调 loss / output head / h192-h256 宽度或 decision-score 对齐，让 epoch4 rank/spread 信号转化为稳定 top-K；multi-seed、score-backtest bridge、candidate matrix、execution-candidate 仍按用户意愿后置。
