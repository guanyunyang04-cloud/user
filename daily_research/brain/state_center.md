...
- 当前 continuous_policy 研究主线已从 r14 直接动作值仲裁推进到 r15 direct-action-preserving translation / release-funding repair；r15 已完成代码落地、dry-run 与短窗 smoke/audit，尚未启动正式 bounded study。
- r11/r11b/r12/r13/r14/r15 全部分支仍是 `shadow_only` 或 research 入口；不得把 `result_value_v10`、`alpha_result_value_budget_split_v11/v12/v13/v14/v15` 或任何 confirm / smoke 分支直接解释成 promotion / live 切换证据。
- 本轮已完成历史归档压缩、r12/r13/r14 shadow study、月度收益评价接入、r14 repaired confirm 写回与 r15 直接动作保真翻译入口；未切换 live，未改写 promotion gate。
...
- 已新增 `split_heads_direct_action_translation_r15`、`alpha_result_value_budget_split_v15`、`cash_constraint_direct_action_guard_v8` 与 `direct_action_translation_v1`，目标是让订单/预算层优先保留 direct action intent，并把 funding sell 绑定到 direct release authorization / protected hold 证据。
- r15 短窗 smoke 事实：复用 r14 `confirm_01` artifact，在 `20260102-20260228`、`cash_constraint_direct_action_guard_v8` 下评估通过；`direct_action_intent_preserved_share = 0.9191`、`direct_action_order_translation_conflict_rate = 0.0515`、`direct_action_value_low_margin_share = 0.8676`、`deploy_intent_realized_rate = 0.7083`，审计仍判定 `direct_action_intent_not_preserved` 与 `deploy_intent_not_executable` 为高优先级瓶颈。该 smoke 只证明链路可运行，不是 promotion 证据。
...
## 当前优先级
- 冻结 live 默认执行，不做静默切换。
- 继续把 r11/r11b/r12/r13/r14/r15 视为 research / shadow 证据，而不是 production 证据。
- 后续若继续推进 continuous_policy，应优先把 r15 从 smoke 推进到 bounded shadow study，重点验证 `direct_action_intent_preserved_share`、`direct_action_value_low_margin_share`、`deploy_intent_realized_rate` 与 release/funding 责任链；单纯继续调高 release loss 已不是最高优先级。
...
- 当前任何 r11/r11b/r12/r13/r14/r15 结果都不改变 active execution artifact。
...
## 推荐下一步
- 若继续研究：直接执行 `split_heads_direct_action_translation_r15` bounded shadow study；先看 dry-run 4 条组合，再按 objective `direct_action_translation_v1` 做 screening/confirm。
- r14 已证明“日级连续决策直接由动作值仲裁”有价值；r15 已把订单/预算翻译层保留 direct action intent 的代码、指标和审计入口接通，下一步专项降低 `direct_action_value_low_margin_share`、deploy intent 未实现率与 held-side funding rebalance 过度依赖。
