...
- v2 horizon train-contract scout 已把 posthoc `penalty_30d_0p005` 形状内化为 `horizon_30d_soft_penalty_v1` loss/output 合同：raw utility 保留，calibrated utility 单独落盘，`pred_decision_score/trade_utility_score/pred_best_horizon` 使用 30d soft penalty。Seed7 `forecast_test_confirmed` 且除 seed_count 外通过 rank/spread/hit/month/concentration 检查，30d concentration `0.024208`；但单 seed 只能证明合同可行，不能替换 strict v2 baseline 或进入执行候选，下一步必须做 seed11/19 confirmation。
