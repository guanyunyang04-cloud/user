...
- 2026-06-01 起，path_policy / multi-horizon 当前新主线切换为 `daily_research_v2_research_reset`：旧 Stage 2.8 / Stage 3A-3G / short_v5b 作为历史参考和方法库保留，不再作为必须复刻的当前目标。
...
- `daily_research_v2_score_backtest_bridge`：v2 reset 的研究输出到执行候选评估入口；把三 seed `pred_decision_score` 集成为 ensemble score panel 并接 shared candidate backtest engine。该 bridge 不是 production target-weight panel，也不是 active/default 解冻依据。
- `daily_research_v2_candidate_review_matrix`：v2 reset 的执行候选矩阵审查入口；对 score-to-weight mapping 的 holding count、max weight、rebalance、成本和风险 overlay 做 research-only shared backtest matrix。首轮 `v2_candidate_review_matrix_20260602_01` 仅测试 holding count `10/20/30`，`0/3` promotion-review eligible，说明当前 blocker 是月度稳定、负月、坏月和回撤，而不是 bridge 是否可运行。
- `daily_research_v2_bad_month_attribution`：v2 reset 的执行候选坏月归因入口；读取 candidate backtest 的 target-weight/score/equity/月度产物和 explicit lake Close 面板，输出 previous-weight x close-return 近似贡献。首轮 `v2_bad_month_attribution_20260602_01` 显示 2024-08 最差月主要来自高分/高权重桶，不是低分尾部仓位。
- `daily_research_v2_risk_overlay_candidate_matrix`：v2 reset 的风险 overlay 候选矩阵；在 bad-month attribution 后测试 max weight、rebalance frequency 和 market-regime filter。首轮 `v2_candidate_review_matrix_risk_overlay_20260602_01` 为 `0/18` promotion-review eligible；粗粒度 regime filter 可降回撤但明显牺牲 excess return / Sharpe。
- `daily_research_v2_selective_throttle_matrix`：v2 reset 的选择性历史状态降分候选矩阵；用 explicit lake Close 计算近期涨幅和波动横截面分位，对 adverse-state 高分票做 research-only score penalty 后进入 shared backtest。首轮 `v2_selective_throttle_matrix_narrow_20260602_01` 为 `0/9` promotion-review eligible；selective throttle 显著改善 Sharpe / drawdown，但月稳仍未闭合。
- `daily_research_v2_state_sizing_matrix`：v2 reset 的二阶段路径状态控仓候选矩阵；读取已完成候选的 aligned target-weight / score / equity 产物，按自身 excess-equity drawdown 或 previous-month-loss 状态缩放 gross target weights，再以 direct target-weight panel 回测。首轮 narrow + light probe 共 `0/20` promotion-review eligible，说明全局路径状态控仓不是当前月稳 blocker 的主修复点。
- `daily_research_v2_post_throttle_bad_month_attribution`：v2 reset 的 selective-throttle 后局部坏月归因；对剩余负月 `2024-03/2024-06/2024-08` 输出 recent return、volatility、reversal 与 score 交叉桶，结论是下一步应做 per-symbol/local cap、score x volatility/reversal sizing 和 weight redistribution，而不是继续全局降风险。
- `daily_research_v2_local_risk_cap_matrix`：v2 reset 的局部风险 target-weight overlay；基于 candidate target weights、score context 和 explicit lake Close 的 previous-close-derived local state，对触发风险的已持仓降权并在未触发风险持仓内重分配。首轮 narrow matrix `0/4` promotion-review eligible，说明 local cap 有效保留信号质量但不能单独闭合月稳 gate。
- `daily_research_v2_local_risk_cap_targeted_matrix`：v2 local cap 的 targeted follow-up；分离 `score_volatility` 与 `score_reversal` 并比较 `source_weight` / `score` 重分配。`score_volatility` 明显占优，但 `0/16` promotion-review eligible，说明 target-weight overlay 已接近当前 score panel 的修复上限。
- `daily_research_v2_local_state_input_scout`：v2 reset 的局部状态模型输入前移实验；`raw_kline_context_v2_tradeable_local_state_v1` 三 seed scout 为 `near_pass`，rank/spread/hit 全正且 hit lift mean 高于当前 strict baseline，但 `negative_month_count_max=4`，不能替换 pass-grade v2 baseline，也不能进入执行候选。
...
- v2 score backtest bridge 可用于 execution-candidate review，但不能单凭一个 score-to-weight 映射解冻执行端；必须继续检查月度稳定性、坏月归因、drawdown、turnover/cost drag、风险约束和多参数 candidate matrix。
- v2 candidate review matrix 的 promotion-review gate 当前至少包括：monthly positive ratio、negative month count、max drawdown、excess Sharpe、deep bad month flag、annual return cost drag。首轮 holding-count 矩阵未通过时，下一步应优先做 bad-month attribution、risk/month-stability overlay 和候选映射扩展，而不是直接讨论 execution unfreeze。
- v2 bad-month attribution 是候选设计诊断，不是精确 PnL 归因；其默认方法是 previous target weight x close-to-close return。若最差月负贡献集中在高分/高权重桶，下一步应优先测试 max-weight 降权、risk/month-state overlay、高置信 throttle、反转/波动 guard，而不是把问题误判为“持仓数不对”或“低分尾部污染”。
- v2 risk overlay 不能只靠 coarse market-regime filter：若全局 risk-off 可明显降 drawdown 但同时压掉 excess Sharpe，说明该 overlay 太钝；下一步应做 selective high-confidence throttle、reversal/volatility guard、state-specific sizing，而不是继续放大粗粒度 regime 开关矩阵。
- v2 selective-throttle 后的剩余负月应按局部状态处理：`2024-08` 是 high-score/high-weight 与 high-volatility/reversal 交互损失，`2024-06` 是 mid/high score、mid weight、高近期涨幅和低波动错觉，`2024-03` 混合 benchmark/cost/timing 与高波动/弱反转拖累。下一轮候选矩阵应优先测试局部 cap 和权重再分配。
- v2 local risk cap 的首轮结果说明“局部风控方向合理但不充分”：保持 gross 与 max-weight 约束、消除 deep bad month、Sharpe 维持 `1.65-1.71` 仍不能把 `8/11` 正月和 `3` 负月推进到 promotion gate。下一步需要更细条件化 local cap 或把 local state 前移到模型输入 / loss，而不是继续叠加同类简单 overlay。
- v2 targeted local cap 进一步说明：当前剩余 blocker 主要是 high-score local-volatility 暴露，但 simple overlay 无法把月稳 gate 从 `8/11`、`3` 负月推进到通过。下一步优先级应从执行候选 overlay 转向模型输入 / score calibration / loss-output 约束，把 local volatility/adverse-state 变成模型可学习或可校准的合同。
- v2 score-state calibration 已验证 validation-selected fixed score penalty 协议可运行，但首轮 `score_volatility p0.02` / `score_volatility_or_reversal p0.02` probe 仍为 `0/2` promotion-review eligible，且月稳退回 `7/11` 正月、`4` 负月并保留 deep bad month / concentrated positive month flags；后续不应继续堆同类固定 penalty，应升级到显式 local-state 输入、校准头或 loss/output 约束。
- v2 local-state 输入合同候选为 `raw_kline_context_v2_tradeable_local_state_v1`：它继承 v2 amount checked 与 no-alpha/no-score 合同，新增 `local_state_context` 的 recent return、volatility、reversal、drawdown、cross-section rank/z 和 adverse-state interaction 特征。该 profile 是训练前输入合同证据，不是模型质量证据；后续必须用同 dataset/pool/model/loss/date split 与当前 strict baseline 做 narrow scout 和多 seed gate。
- v2 local-state 输入的 3-seed scout 说明“把局部状态前移到模型输入”方向有价值但不充分：hit lift mean/min 改善为 `0.018857/0.014302` 且全 seed 为正，但 monthly stability 从当前 strict baseline 的 `negative_month_count_max=2` 退化到 `4`。后续应优先研究 loss/output calibration、horizon/month-state constraints 或更结构化 input，而不是把该 profile 直接作为新 baseline。
- v2 local-state loss calibration 的 3-seed scout 说明 loss/output 侧能修复 local-state 月稳并增强 hit lift，但会把 predicted horizon 再次推向长 horizon：`horizon_entropy_regularized_v1` 的 rank/spread/hit/months 全过线，hit lift mean/min `0.030121/0.025153`，negative months max `2`，但 30d concentration mean `0.805152` 未过 gate。因此该线下一步必须先做 horizon-concentration repair / output calibration，不能替换 strict v2 baseline，也不能直接进入执行候选。
- v2 horizon concentration repair scout 说明 30d concentration 可以被输出校准独立修复：对 `horizon_entropy_regularized_v1` 三 seed prediction 做 posthoc `penalty_30d_0p005` 后，30d concentration 从 `0.805152` 降至 `0.109297`，rank/spread/hit/months 基本保持过线。但该证据是 posthoc output repair，不是训练侧模型 gate pass；下一步应把该形状内化为 horizon soft penalty、concentration cap 或 calibrated horizon head，再决定是否进入 score-backtest bridge。
- v2 horizon train-contract scout 已把 posthoc `penalty_30d_0p005` 形状内化为 `horizon_30d_soft_penalty_v1` loss/output 合同：raw utility 保留，calibrated utility 单独落盘，`pred_decision_score/trade_utility_score/pred_best_horizon` 使用 30d soft penalty。Seed7/11/19 confirmation 已完成，三 seed forecast gate 为 `pass`：rank/spread/hit/months/concentration 全过，hit lift mean/min `0.023851/0.015980`，30d concentration mean `0.042104`。这使该分支成为 research-only score-backtest bridge 候选，但不能直接替换 strict v2 baseline、不能进入 execution promotion 或解冻执行端。
- v2 selective throttle 的高 Sharpe / 低回撤结果只能说明历史状态降分方向有效，不能越过月稳 gate。若 positive month ratio 仍低于 `0.75` 或 negative months 仍大于 `2`，即使 excess Sharpe 大于 `1.5`，也必须继续做 month-state / drawdown-state sizing、post-throttle attribution 和稳定性修复，不能进入 execution unfreeze。
- v2 state sizing 的负结果同样是有效证据：若全局 drawdown / previous-month-loss 缩放不能减少负月，且明显降低 excess Sharpe 或引入 deep bad month，则后续不要继续放大同类全局路径控仓矩阵；应转向局部暴露修复，包括 per-symbol reversal/volatility bucket sizing、sector/liquidity/volatility/horizon caps、以及 post-throttle 剩余负月归因。
