...
- `daily_only_summary_v2_ohlcva_aux` is an explicit volume/amount auxiliary-supervision profile: it predicts a 6D OHLCVA auxiliary path but keeps `future_path` as 4D OHLC and keeps summary/value/rank on price-only `path_trade_value_v2`.
