...
from daily_research.continuous_policy.tests.portfolio_daily_fixtures import (
    _FakeDailyModel,
    _FakePolicyModel,
    _base_policy,
    _healthy_semantic_conflicts,
    _protocol_summary,
    _trial,
)
...
class PortfolioDailyStrategyContractsTest(unittest.TestCase):
    ...
    def test_study_scoring_exports_native_target_validity_metrics(self) -> None:
        scored = _score_protocol_summary(
            _protocol_summary(
                continuity={
                    "allocation_layer_native_target_used": 0.975,
                    "allocation_layer_native_fallback_used": 0.025,
                    "allocation_layer_source_target_count": 3,
                    "allocation_layer_source_executable_candidate_count": 8,
                    "native_negative_delta_count": 4,
                    "native_receiver_executable_mask_count": 12,
                    "native_positive_delta_count": 5,
                    "native_positive_delta_unsupported_share": 0.20,
                    "native_receiver_mask_mismatch_count": 1,
                    "native_source_target_count": 2,
                    "native_target_valid": 0.975,
                    "native_target_constraint_violations": 1,
                    "native_target_invalid_sum_count": 0,
                    "native_target_invalid_turnover_count": 0,
                    "native_target_invalid_cap_count": 1,
        ...
        metrics = scored["primary_metrics"]
        self.assertEqual(metrics["native_target_valid"], 0.975)
        self.assertEqual(metrics["allocation_layer_native_fallback_used"], 0.025)
    ...
    def test_r52_native_target_source_delta_bypasses_legacy_source_candidate_in_simulator(self) -> None:
        state = PortfolioState(
            cash_weight=0.80,
            holdings={"HELD": HoldingState(weight=0.20, entry_price=10.0, peak_price=10.0)},
            max_positions=4,
        ...
        policy = pd.DataFrame(
            {
                "action_label": ["skip", "skip", "skip"],
                "action_strength": [0.0, 0.0, 0.0],
                "portfolio_daily_receiver_score": [0.0, 0.10, 0.0],
                "portfolio_daily_source_score": [0.0, 0.0, 0.0],
                "portfolio_daily_cash_score": [0.20, 0.20, 0.20],
                "portfolio_daily_receiver_executability": [0.0, 1.0, 0.0],
                "portfolio_daily_source_executability": [0.0, 0.0, 0.0],
                "portfolio_daily_receiver_add_headroom": [0.0, 0.30, 0.30],
                "portfolio_daily_source_release_capacity": [0.0, 0.0, 0.0],
                "portfolio_daily_receiver_executable_candidate": [0.0, 1.0, 0.0],
                "portfolio_daily_source_executable_candidate": [0.0, 0.0, 0.0],
                "portfolio_daily_source_candidate": [0.0, 0.0, 0.0],
                "portfolio_daily_target_weight": [0.10, 0.18, 0.0],
                "portfolio_daily_target_delta": [-0.10, 0.18, 0.0],
                "portfolio_daily_target_cash_weight": [0.72, 0.72, 0.72],
                "portfolio_daily_target_turnover": [0.28, 0.28, 0.28],
    ...
    def test_r52_native_target_held_add_does_not_invalidate_receiver_support(self) -> None:
        state = PortfolioState(
            cash_weight=0.90,
            holdings={"HELD": HoldingState(weight=0.10, entry_price=10.0, peak_price=10.0)},
            max_positions=4,
            max_position_weight=0.30,
            turnover_limit=0.40,
        )
        prices = pd.Series({"HELD": 10.0, "OTHER": 10.0})
        policy = pd.DataFrame(
            {
                "action_label": ["skip", "skip"],
                "action_strength": [0.0, 0.0],
                ...
                "portfolio_daily_source_score": [0.0, 0.0],
                "portfolio_daily_cash_score": [0.20, 0.20],
                "portfolio_daily_receiver_executable_candidate": [0.0, 0.0],
                "portfolio_daily_source_executable_candidate": [0.0, 0.0],
                "portfolio_daily_target_weight": [0.15, 0.0],
                "portfolio_daily_target_delta": [0.05, 0.0],
                "portfolio_daily_target_cash_weight": [0.85, 0.85],
            },
            index=prices.index,
        )
    ...
    def test_r53_scoring_does_not_penalize_closed_budget_for_inactive_receiver(self) -> None:
        scored = _score_protocol_summary(
            _protocol_summary(
                metrics={
                    "annual_return": 0.19,
                    "sharpe": 1.08,
                    "monthly_return_mean": 0.012,
                    "monthly_consistency_score": 0.58,
                    "max_drawdown": -0.10,
                    "avg_gross_exposure": 0.81,
                },
                semantic={
                    "avg_gross_exposure_target": 0.747,
                    "portfolio_daily_exposure_utilization": 1.08,
                    "portfolio_daily_actual_cash_weight_mean": 0.19,
                    "portfolio_daily_actual_gross_exposure_mean": 0.81,
                    "portfolio_daily_deployable_idle_cash_mean": 0.005,
                    "portfolio_daily_cash_semantics_mismatch": 0.0,
                    "portfolio_daily_target_sum_gap": 0.005,
                    "portfolio_daily_cash_funded_deploy_amount_mean": 0.01,
                    ...
                    "portfolio_daily_source_target_count": 0.0,
                    "portfolio_daily_source_realized_sell_rate": 0.0,
                    "portfolio_daily_receiver_candidate_without_target_day_share": 1.0,
                    "receiver_activity_required": 0.0,
                    "budget_closed": 1.0,
                },
            ),
            objective_profile="end_to_end_allocation_layer_v1",
        ...
        perf = scored["score_breakdown"]["performance"]
        stability = scored["score_breakdown"]["stability"]
        self.assertEqual(perf.get("portfolio_daily_v2_cash_funded_deploy_gate", 0.0), 0.0)
        self.assertEqual(perf.get("portfolio_daily_v2_unused_receiver_headroom_gate", 0.0), 0.0)
        self.assertEqual(perf.get("portfolio_daily_v2_receiver_activity_gate", 0.0), 0.0)
        self.assertEqual(perf.get("portfolio_daily_receiver_candidate_without_target_penalty", 0.0), 0.0)
        self.assertEqual(stability.get("portfolio_daily_v2_cash_funded_deploy_gate", 0.0), 0.0)
        self.assertEqual(stability.get("portfolio_daily_v2_unused_receiver_headroom_gate", 0.0), 0.0)
    ...
    def test_r53_scoring_still_blocks_unfilled_cash_budget(self) -> None:
        scored = _score_protocol_summary(
            _protocol_summary(
                metrics={
                    "annual_return": 0.04,
        ...
        perf = scored["score_breakdown"]["performance"]
        self.assertLess(perf.get("portfolio_daily_v2_cash_funded_deploy_gate", 0.0), 0.0)
        self.assertLess(perf.get("portfolio_daily_v2_unused_receiver_headroom_gate", 0.0), 0.0)
        self.assertLess(perf.get("portfolio_daily_v2_receiver_activity_gate", 0.0), 0.0)
        self.assertLess(perf.get("portfolio_daily_receiver_candidate_without_target_penalty", 0.0), 0.0)
    ...
    def test_r54_predict_policy_exports_target_weight_intent_mode(self) -> None:
        ...
        artifact = SimpleNamespace(
            ...
            training_diagnostics={
                ...
                "supports_portfolio_unified_allocation_heads": True,
                "supports_portfolio_allocation_objective_consolidation_heads": True,
                "supports_allocation_intent_v2_mode": True,
            },
        )
        ...
        self.assertEqual(global_targets["allocation_core_v2_mode"], 1.0)
        self.assertEqual(global_targets["allocation_intent_v2_mode"], 1.0)
        self.assertIn("portfolio_daily_target_weight_intent", policy.columns)
        self.assertIn("allocation_intent_v2_mode", policy.columns)
        self.assertTrue(bool((policy["allocation_intent_v2_mode"] == 1.0).all()))
        self.assertTrue(bool((policy["portfolio_daily_target_weight_intent"] >= 0.0).all()))

    def test_r54_simulator_derives_execution_from_target_weight_intent(self) -> None:
        state = PortfolioState(
            cash_weight=0.72,
            holdings={"HELD": HoldingState(weight=0.28, entry_price=10.0, peak_price=10.0)},
            max_positions=5,
        ...
        policy = pd.DataFrame(
            {
                "action_label": ["skip", "skip", "skip", "skip"],
                "action_strength": [0.0, 0.0, 0.0, 0.0],
                "portfolio_daily_target_weight_intent": [0.28, 0.30, 0.27, 0.00],
                "portfolio_daily_receiver_score": [0.0, 0.95, 0.90, 0.0],
                "portfolio_daily_source_score": [0.20, 0.0, 0.0, 0.0],
                "portfolio_daily_cash_score": [0.05, 0.05, 0.05, 0.05],
                "portfolio_daily_receiver_executability": [0.0, 1.0, 1.0, 0.0],
                "portfolio_daily_source_executability": [1.0, 0.0, 0.0, 0.0],
                "portfolio_daily_receiver_add_headroom": [0.0, 0.30, 0.30, 0.30],
                "portfolio_daily_source_release_capacity": [0.10, 0.0, 0.0, 0.0],
                "portfolio_daily_receiver_executable_candidate": [0.0, 1.0, 1.0, 0.0],
                "portfolio_daily_source_executable_candidate": [1.0, 0.0, 0.0, 0.0],
                "allocation_core_v2_mode": [1.0, 1.0, 1.0, 1.0],
                "allocation_intent_v2_mode": [1.0, 1.0, 1.0, 1.0],
            },
            index=prices.index,
        )
        ...
        self.assertEqual(result.diagnostics["allocation_layer_core_v2_used"], 1.0)
        self.assertEqual(result.diagnostics["allocation_intent_v2_mode_used"], 1.0)
        self.assertGreaterEqual(float(result.weights.sum()), 0.75)
        self.assertLessEqual(result.diagnostics["intent_translation_conflict_rate"], 0.10)
        opened = {
            item["stock"]: item
            for item in result.actions
            if item["stock"] in {"RECV_A", "RECV_B"}
        }
        self.assertEqual(opened["RECV_A"]["execution_action"], "open")
        self.assertEqual(opened["RECV_A"]["portfolio_daily_effective_model_action"], "open")
        self.assertEqual(opened["RECV_A"]["semantic_translation_reason"], "allocation_intent_v2_delta")

    def test_r54_intent_translation_ignores_deadband_micro_delta(self) -> None:
        state = PortfolioState(
            cash_weight=0.43702,
            holdings={
                "H1": HoldingState(weight=0.28149, entry_price=10.0, peak_price=10.0),
                "H2": HoldingState(weight=0.28149, entry_price=10.0, peak_price=10.0),
            },
            max_positions=5,
            max_position_weight=0.30,
            turnover_limit=0.10,
        )
        prices = pd.Series({"H1": 10.0, "H2": 10.0})
        policy = pd.DataFrame(
            {
                "action_label": ["skip", "skip"],
                "action_strength": [0.0, 0.0],
                "portfolio_daily_target_weight_intent": [0.28, 0.28],
                "portfolio_daily_receiver_score": [0.0, 0.0],
                "portfolio_daily_source_score": [0.0, 0.0],
                "portfolio_daily_receiver_executability": [0.0, 0.0],
                "portfolio_daily_source_executability": [0.0, 0.0],
                "portfolio_daily_receiver_executable_candidate": [0.0, 0.0],
                "portfolio_daily_source_executable_candidate": [0.0, 0.0],
                "allocation_core_v2_mode": [1.0, 1.0],
        ...
        self.assertEqual(result.diagnostics["allocation_intent_v2_mode_used"], 1.0)
        self.assertEqual(result.diagnostics["intent_translation_conflict_count"], 0)
        self.assertEqual(result.diagnostics["intent_translation_conflict_rate"], 0.0)
        self.assertTrue(all(item["weight_change_action"] == "hold" for item in result.actions))
    ...
    def test_r52_native_source_delta_ignores_legacy_source_candidate_mask(self) -> None:
        outputs = {
            "portfolio_daily_allocation_weight_logit": torch.tensor([0.0, -2.0], dtype=torch.float32),
            "portfolio_daily_cash_reserve_logit": torch.tensor([4.0, 4.0], dtype=torch.float32),
            "portfolio_daily_allocation_risk_buffer_logit": torch.tensor([0.0, 0.0], dtype=torch.float32),
        }
        targets = {
            "date_code": torch.tensor([0.0, 0.0], dtype=torch.float32),
            "current_weight": torch.tensor([0.30, 0.0], dtype=torch.float32),
            "portfolio_daily_receiver_candidate_mask": torch.tensor([0.0, 0.0], dtype=torch.float32),
            "portfolio_daily_source_candidate_mask": torch.tensor([0.0, 0.0], dtype=torch.float32),
            "portfolio_daily_receiver_executable_candidate": torch.tensor([0.0, 0.0], dtype=torch.float32),
            "portfolio_daily_source_executable_candidate": torch.tensor([0.0, 0.0], dtype=torch.float32),
            "gross_exposure_target": torch.tensor([0.12, 0.12], dtype=torch.float32),
            "turnover_budget": torch.tensor([0.50, 0.50], dtype=torch.float32),
            "max_position_weight_target": torch.tensor([0.50, 0.50], dtype=torch.float32),
        }

        projection = _project_native_allocation_vector(outputs, targets, return_terms=True)

        self.assertLess(float(projection["portfolio_daily_target_delta"][0]), -0.05)
        self.assertGreater(float(projection["portfolio_daily_native_source_score"][0]), 0.0)
        self.assertGreater(float(projection["native_negative_delta_count"]), 0.0)
        self.assertGreater(float(projection["native_source_candidate_count"]), 0.0)
        self.assertGreater(float(projection["native_source_blocked_by_legacy_mask"]), 0.0)

    def test_r52_native_projection_concentrates_tiny_releases_above_execution_threshold(self) -> None:
        row_count = 10
        outputs = {
            "portfolio_daily_allocation_weight_logit": torch.zeros(row_count, dtype=torch.float32),
            "portfolio_daily_cash_reserve_logit": torch.full((row_count,), 2.0, dtype=torch.float32),
            "portfolio_daily_allocation_risk_buffer_logit": torch.zeros(row_count, dtype=torch.float32),
        }
        targets = {
            "date_code": torch.zeros(row_count, dtype=torch.float32),
            "current_weight": torch.full((row_count,), 0.02, dtype=torch.float32),
            "portfolio_daily_receiver_candidate_mask": torch.zeros(row_count, dtype=torch.float32),
            "portfolio_daily_source_candidate_mask": torch.zeros(row_count, dtype=torch.float32),
            "gross_exposure_target": torch.full((row_count,), 0.19, dtype=torch.float32),
            "turnover_budget": torch.full((row_count,), 0.05, dtype=torch.float32),
            "max_position_weight_target": torch.full((row_count,), 0.05, dtype=torch.float32),
        }

        projection = _project_native_allocation_vector(outputs, targets, return_terms=True)

        self.assertGreater(float(projection["source_flow_mean"]), 0.0)
        self.assertGreater(float(projection["native_negative_delta_count"]), 0.0)
        self.assertGreater(float(projection["native_source_candidate_count"]), 0.0)
    ...
    def test_r52c_native_receiver_executable_mask_keeps_held_and_blocks_flat_unsupported(self) -> None:
        mask = build_native_receiver_executable_mask(
            current_weight=torch.tensor([0.20, 0.0, 0.0], dtype=torch.float32),
            receiver_candidate_mask=torch.tensor([0.0, 1.0, 1.0], dtype=torch.float32),
            receiver_executable_candidate=torch.tensor([0.0, 1.0, 0.0], dtype=torch.float32),
        )

        self.assertTrue(torch.equal(mask, torch.tensor([1.0, 1.0, 0.0])))

    def test_r52c_projection_uses_executable_receiver_mask_as_hard_support(self) -> None:
        outputs = {
            "portfolio_daily_allocation_weight_logit": torch.tensor([0.0, 1.0, 9.0, 2.0], dtype=torch.float32),
            "portfolio_daily_cash_reserve_logit": torch.full((4,), -4.0, dtype=torch.float32),
            "portfolio_daily_allocation_risk_buffer_logit": torch.full((4,), -4.0, dtype=torch.float32),
        }
        targets = {
            "date_code": torch.zeros(4, dtype=torch.float32),
            "current_weight": torch.tensor([0.20, 0.0, 0.0, 0.0], dtype=torch.float32),
            "portfolio_daily_receiver_candidate_mask": torch.tensor([0.0, 1.0, 1.0, 1.0], dtype=torch.float32),
            "portfolio_daily_source_candidate_mask": torch.zeros(4, dtype=torch.float32),
            "portfolio_daily_receiver_executable_candidate": torch.tensor([0.0, 1.0, 0.0, 1.0], dtype=torch.float32),
            "gross_exposure_target": torch.full((4,), 0.42, dtype=torch.float32),
            "turnover_budget": torch.full((4,), 0.30, dtype=torch.float32),
            "max_position_weight_target": torch.full((4,), 0.24, dtype=torch.float32),
            "portfolio_daily_allocation_cash_deployment_target": torch.full((4,), 0.95, dtype=torch.float32),
        }

        projection = _project_native_allocation_vector(outputs, targets, validity_first=True, return_terms=True)

        self.assertAlmostEqual(float(projection["portfolio_daily_target_weight"][2]), 0.0, places=6)
        self.assertEqual(float(projection["native_target_invalid_reason_unsupported_receiver"]), 0.0)
        self.assertEqual(float(projection["simulator_exact_unsupported_receiver_count_proxy"]), 0.0)
        self.assertGreater(float(projection["simulator_exact_native_valid_proxy"]), 0.99)

    def test_r52c_simulator_reports_receiver_mask_mismatch_without_relaxing_hard_gate(self) -> None:
        state = PortfolioState(
            cash_weight=0.90,
            holdings={"HELD": HoldingState(weight=0.10, entry_price=10.0, peak_price=10.0)},
            max_positions=4,
        ...
        prices = pd.Series({"HELD": 10.0, "BAD": 10.0, "GOOD": 10.0})
        policy = pd.DataFrame(
            {
                "action_label": ["skip", "skip", "skip"],
                "action_strength": [0.0, 0.0, 0.0],
                "portfolio_daily_receiver_score": [0.0, 0.0, 0.50],
                "portfolio_daily_source_score": [0.0, 0.0, 0.0],
                "portfolio_daily_cash_score": [0.20, 0.20, 0.20],
                "portfolio_daily_receiver_executable_candidate": [0.0, 0.0, 1.0],
                "portfolio_daily_source_executable_candidate": [0.0, 0.0, 0.0],
                "portfolio_daily_target_weight": [0.10, 0.12, 0.12],
                "portfolio_daily_target_delta": [0.0, 0.12, 0.12],
                "portfolio_daily_target_cash_weight": [0.66, 0.66, 0.66],
            },
            index=prices.index,
        )
        ...
        self.assertEqual(result.diagnostics["allocation_layer_native_target_used"], 0.0)
        self.assertEqual(result.diagnostics["allocation_layer_native_fallback_used"], 1.0)
        self.assertGreater(result.diagnostics["native_target_invalid_unsupported_receiver_count"], 0)
        self.assertGreater(result.diagnostics["native_receiver_mask_mismatch_count"], 0)
        self.assertGreater(result.diagnostics["native_positive_delta_unsupported_share"], 0.0)

    def test_r52c_projection_output_is_accepted_by_simulator_receiver_gate(self) -> None:
        outputs = {
            "portfolio_daily_allocation_weight_logit": torch.tensor([0.0, 6.0, 9.0], dtype=torch.float32),
            "portfolio_daily_cash_reserve_logit": torch.full((3,), -4.0, dtype=torch.float32),
            "portfolio_daily_allocation_risk_buffer_logit": torch.full((3,), -4.0, dtype=torch.float32),
        }
        targets = {
            "date_code": torch.zeros(3, dtype=torch.float32),
            "current_weight": torch.tensor([0.20, 0.0, 0.0], dtype=torch.float32),
            "portfolio_daily_receiver_candidate_mask": torch.tensor([0.0, 1.0, 1.0], dtype=torch.float32),
            "portfolio_daily_source_candidate_mask": torch.zeros(3, dtype=torch.float32),
            "portfolio_daily_receiver_executable_candidate": torch.tensor([0.0, 1.0, 0.0], dtype=torch.float32),
            "gross_exposure_target": torch.full((3,), 0.32, dtype=torch.float32),
            "turnover_budget": torch.full((3,), 0.35, dtype=torch.float32),
            "max_position_weight_target": torch.full((3,), 0.30, dtype=torch.float32),
            "portfolio_daily_allocation_cash_deployment_target": torch.full((3,), 0.95, dtype=torch.float32),
        }
        projection = _project_native_allocation_vector(outputs, targets, validity_first=True, return_terms=True)
        prices = pd.Series({"HELD": 10.0, "GOOD": 10.0, "BAD": 10.0})
        policy = pd.DataFrame(
            {
                "action_label": ["skip", "skip", "skip"],
                "action_strength": [0.0, 0.0, 0.0],
                "portfolio_daily_receiver_score": [0.0, 0.40, 0.0],
                "portfolio_daily_source_score": [0.0, 0.0, 0.0],
                "portfolio_daily_cash_score": [0.20, 0.20, 0.20],
                "portfolio_daily_receiver_executable_candidate": [0.0, 1.0, 0.0],
                "portfolio_daily_source_executable_candidate": [0.0, 0.0, 0.0],
                "portfolio_daily_target_weight": projection["portfolio_daily_target_weight"].detach().numpy(),
                "portfolio_daily_target_delta": projection["portfolio_daily_target_delta"].detach().numpy(),
                "portfolio_daily_target_cash_weight": projection["portfolio_daily_target_cash_weight"].detach().numpy(),
            },
            index=prices.index,
        )
        state = PortfolioState(
        ...
        self.assertEqual(result.diagnostics["allocation_layer_native_target_used"], 1.0)
        self.assertEqual(result.diagnostics["native_target_invalid_unsupported_receiver_count"], 0)
        self.assertEqual(result.diagnostics["native_receiver_mask_mismatch_count"], 0)
    ...
    def test_r52_day_set_loss_penalizes_receiver_demand_without_native_source(self) -> None:
        terms = _portfolio_day_set_native_allocation_vector_loss(
            ...
            {
                "current_weight": torch.tensor([[0.0, 0.0, 0.0]], dtype=torch.float32),
                "portfolio_daily_receiver_candidate_mask": torch.tensor([[0.0, 1.0, 1.0]], dtype=torch.float32),
                "portfolio_daily_source_candidate_mask": torch.tensor([[0.0, 0.0, 0.0]], dtype=torch.float32),
                "portfolio_daily_receiver_executable_candidate": torch.tensor([[0.0, 1.0, 1.0]], dtype=torch.float32),
                "portfolio_daily_source_executable_candidate": torch.tensor([[0.0, 0.0, 0.0]], dtype=torch.float32),
                "gross_exposure_target": torch.tensor([0.60], dtype=torch.float32),
                "turnover_budget": torch.tensor([0.70], dtype=torch.float32),
                "max_position_weight_target": torch.tensor([0.40], dtype=torch.float32),
                "portfolio_daily_allocation_cash_deployment_target": torch.tensor([[0.95, 0.95, 0.95]], dtype=torch.float32),
            },
            torch.tensor([[True, True, True]], dtype=torch.bool),
            return_terms=True,
        )
    ...
    def test_r61_core_v4_release_first_policy_connects_source_receiver_and_trace(self) -> None:
        from daily_research.continuous_policy.portfolio_simulator import HoldingState, PortfolioState
        ...
        policy = pd.DataFrame(
            {
                ...
                "action_label": ["reduce", "open", "open"],
                "portfolio_daily_target_weight_intent": [0.12, 0.24, 0.18],
                "portfolio_daily_target_delta_intent": [-0.18, 0.24, 0.18],
                "portfolio_daily_release_first_intent": [0.90, 0.0, 0.0],
                "release_first_intent_score": [0.90, 0.0, 0.0],
                "release_first_action_hint": ["reduce", "hold", "hold"],
                "release_first_block_reason": ["none", "not_held", "not_held"],
                "portfolio_daily_source_score": [0.92, 0.0, 0.0],
                "portfolio_daily_unified_source_score": [0.92, 0.0, 0.0],
                "portfolio_daily_source_release_quality": [0.88, 0.0, 0.0],
                "portfolio_daily_source_economic_block_risk": [0.02, 0.0, 0.0],
                "portfolio_daily_receiver_score": [0.0, 0.95, 0.90],
                "portfolio_daily_unified_receiver_score": [0.0, 0.95, 0.90],
                "portfolio_daily_receiver_executable_candidate": [0.0, 1.0, 1.0],
                "portfolio_daily_source_executable_candidate": [1.0, 0.0, 0.0],
            }
        ).set_index("stock")
        ...
        result = state.step(
            date="2026-05-13",
            prices=pd.Series({"SRC": 10.0, "RCV1": 10.0, "RCV2": 10.0}),
            policy_frame=policy,
            global_targets={
                "gross_exposure_target": 0.42,
                "turnover_budget": 1.00,
                "max_position_weight_target": 0.50,
                "cash_reserve_target": 0.05,
        ...
        self.assertGreaterEqual(result.diagnostics["portfolio_daily_source_target_count"], 1)
        self.assertGreaterEqual(result.diagnostics["portfolio_daily_receiver_target_count"], 1)
