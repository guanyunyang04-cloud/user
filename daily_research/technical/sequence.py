...
class RawSequenceBuilder:
    def __init__(
        ...
        *,
        ...
        self.data = data
        self.daily_raw = open_array(
            data.pack["feature_channels"]["daily_raw"], np.float32
        )
        self.has_bar = open_array(data.pack["masks"]["has_bar"], np.bool_)
...
def _folds(data: TechnicalData) -> list[dict[str, Any]]:
...
def _market_normalization(
    data: TechnicalData, train_rows: np.ndarray
...
def _contract(data: TechnicalData, fold: Mapping[str, Any]) -> dict[str, Any]:
    return {
...
def _task_fingerprint(data: TechnicalData, fold: Mapping[str, Any]) -> str:
    return stable_hash(
        {
            **_contract(data, fold),
...
def load_fold_results() -> tuple[list[dict[str, Any]], pd.DataFrame]:
    ...
    for fold in range(1, 6):
        ...
        if not path.is_file():
            raise TechnicalDataError(f"raw60 fold is missing: {path}")
        ...
        if (
            ...
            raise TechnicalDataError(f"raw60 fold is stale or invalid: {path}")
    ...
    if oof.duplicated("row_position").any():
        raise TechnicalDataError("raw60 OOF row positions overlap")
    return results, oof
