...
def _write_summary(
    ...
    *,
    ...
    summary_payload = {
        ...
        "recent_protocol": str(recent_protocol),
    ...
    lines = [
        ...
        f"- recent 窗口固定为最近一年 `12` 个月，winner 与 companion 都按同一 corrected recent 协议评估：`{recent_protocol}`。",
        "",
        "## Month Plan",
...
def main() -> None:
    ...
    recent_protocol = str(summary.get("recent_validation_protocol", "") or winner_recent_bundle.get("protocol", ""))
    ...
    if winner_score_panel.empty or winner_target_panel.empty:
        raise RuntimeError("Winner corrected recent panels are empty.")
    if companion_target_panel.empty:
        raise RuntimeError("Companion corrected recent target panel is empty.")
    ...
    _write_summary(
        ...
        recent_protocol=recent_protocol,
