...
DEFAULT_ROOT_TAG = "short_alpha_policy_v5_family_formal_review_20260412_r1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a narrow formal family review for policy_v5a / policy_v5b / policy_v5c against the current mainline."
    )
...
def _ensure_profile_run(profile_name: str, args: argparse.Namespace) -> tuple[Path, str]:
    ...
    epoch_budget = resolve_epoch_budget_for_family("short_alpha", manifest_path=str(args.family_epoch_budget_manifest), fallback_epochs=8)
    profile = get_profile(profile_name)
    command = [
        ...
        "--research-time-unit", "calendar_months",
        "--train-end-date", "2025-03-17",
        "--valid-start-date", "2025-03-18",
        "--valid-days", "0",
        "--valid-months", "12",
        "--lookback-window", "120",
        "--batch-size", "256",
        ...
        "--learning-rate", "0.001",
        "--weight-decay", "0.0001",
        "--epochs", str(epoch_budget),
        "--min-epochs", "1",
        "--early-stop-patience", str(max(int(epoch_budget), 8)),
        "--lr-plateau-patience", "4",
        "--lr-plateau-factor", "0.5",
        "--min-improvement", "0.0001",
        "--return-loss-mode", "top_bottom_bce",
        "--return-target-transform", "raw",
        "--score-risk-mode", "subtract",
        "--execution-alignment-mode", "train_eval_auto",
        "--execution-alignment-objective", "robust_composite",
        "--execution-alignment-candidate-profiles", str(args.execution_alignment_candidate_profiles),
        ...
        "--experiment-tag", f"{str(args.root_tag).strip()}/runs/{profile_name}",
    ]
...
def main() -> None:
    ...
    lines = [
        "# Policy V5 Family Formal Review",
        "",
        "## Scope",
    ...
    lines.extend(
        [
            ...
            "",
            "## Decision",
            "- Only a family candidate that preserves the v4 recent edge while materially improving constrained deployability should move on to the next formal/constrained/recent loop.",
        ]
    )
