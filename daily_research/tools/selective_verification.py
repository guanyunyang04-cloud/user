...
from daily_research.tools.brain_platform import PYTHON_EXECUTABLE, WORKSPACE_ROOT
...
ALWAYS_COMMANDS = (
    ...
    "git diff --check",
    f"{PYTHON_EXECUTABLE} daily_research/tools/doc_guard.py check",
    f"{PYTHON_EXECUTABLE} daily_research/tools/brain_integrity_check.py --json",
)

BRAIN_TOOL_TESTS = (
    "daily_research/tools/tests/test_brain_capsule.py",
    "daily_research/tools/tests/test_brain_evidence_registry.py",
    "daily_research/tools/tests/test_brain_rules.py",
    "daily_research/tools/tests/test_brain_workflow_cli.py",
    "daily_research/tools/tests/test_daily_research_brain_skill.py",
)
        ...
        if path.startswith("daily_research/tools/"):
            ...
            if "selective_verification" in path:
                _add_command(
                    ...
                    _pytest_command(
                        [
                            "daily_research/tools/tests/test_selective_verification.py",
                            "daily_research/tools/tests/test_brain_workflow_cli.py",
                        ]
                    ),
                )
            elif Path(path).name.startswith("brain_") or Path(path).name in {"doc_guard.py", "brain_integrity_check.py"}:
                _add_command(selected_commands, _pytest_command(BRAIN_TOOL_TESTS))
            else:
                _add_command(selected_commands, _pytest_command(["daily_research/tools/tests"]))
            continue

        if path.startswith("daily_research/brain/workflows/") or path == "daily_research/brain/workflow_registry.json":
            risk_level = _risk_max(risk_level, "medium")
            _add_command(selected_commands, _pytest_command(BRAIN_TOOL_TESTS))
            continue
