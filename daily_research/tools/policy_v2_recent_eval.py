...
DEFAULT_ROOT_TAG = "short_alpha_policy_v2_recent_eval_20260410_r1"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Recent 12-month validation for short_expert_policy_v2 using an independent "
...
def _notes_by_profile() -> dict[str, str]:
    return {
        ...
        "short_expert_policy_v1": "policy_v1 learned control branch",
...
def _build_summary(recent_df: pd.DataFrame, recent_summary: dict[str, object], output_dir: Path) -> None:
    ...
    policy_v1 = lookup.get("short_expert_policy_v1", {})
    ...
    policy_v1_robust = float(policy_v1.get("recent_monthly_robust_score", 0.0) or 0.0)
    ...
    lines = [
        ...
        "- compared branches: `short_expert_monthly_v1`, `short_expert_policy_v1`, `short_expert_policy_v2`, `state_liquidity_listwise_v1`",
        "",
        "## Direct Answer",
        f"- recent winner: `{winner.get('profile_name', 'n/a')}`",
        f"- policy_v2 monthly robust: `{format_num(policy_v2_robust)}`",
        f"- delta vs policy_v1: `{format_num(policy_v2_robust - policy_v1_robust)}`",
        f"- delta vs current default: `{format_num(policy_v2_robust - current_robust)}`",
        f"- delta vs companion: `{format_num(policy_v2_robust - companion_robust)}`",
        "",
        "## Scoreboard",
    ]
