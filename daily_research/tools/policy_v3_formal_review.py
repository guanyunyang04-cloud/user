...
DEFAULT_ROOT_TAG = "short_alpha_policy_v3_review_20260410_r1"
...
def _resolve_run_dir(run_dir: Path) -> Path:
    ...
    nested_metrics = run_dir / "runs" / "short_expert_policy_v3" / "metrics.json"
    if direct_metrics.exists():
        return run_dir
    if nested_metrics.exists():
        return run_dir / "runs" / "short_expert_policy_v3"
    return run_dir / "runs" / "short_expert_policy_v3"


def _policy_v3_run_dir(root_tag: str) -> Path:
    return _resolve_run_dir(OUTPUT_ROOT / root_tag)


def _ensure_policy_v3_run(args: argparse.Namespace) -> Path:
    run_dir = _policy_v3_run_dir(str(args.root_tag).strip())
    ...
    profile = get_profile("short_expert_policy_v3")
    command = [
...
def main() -> None:
    ...
    policy_v3_run_dir = _ensure_policy_v3_run(args)
    reference_runs = [
        (
    ...
    rows = [_build_row(profile_name, run_dir, source_label) for profile_name, run_dir, source_label in reference_runs]
    ...
    policy_v3_row = scoreboard.loc[scoreboard["profile_name"] == "short_expert_policy_v3"].iloc[0].to_dict()
    ...
    summary = {
        "formal_winner_profile_name": str(winner["profile_name"]),
        "policy_v3_row": policy_v3_row,
    ...
    lines = [
        ...
        f"- policy_v3 execution profile: `{policy_v3_row['execution_alignment_profile']}`",
        f"- policy_v3 monthly robust: `{_format_num(policy_v3_row['monthly_robust_score'])}`",
        f"- delta vs current mainline monthly robust: `{_format_num(float(policy_v3_row['monthly_robust_score']) - float(current_row['monthly_robust_score']))}`",
        f"- delta vs companion monthly robust: `{_format_num(float(policy_v3_row['monthly_robust_score']) - float(companion_row['monthly_robust_score']))}`",
        "",
        "## Scoreboard",
    ]
    ...
    lines.extend(
        [
            "",
            "## Decision",
            "- `policy_v3` must beat both the current mainline and the companion on the formal monthly-first gate before it can move on to recent validation as a default candidate.",
        ]
    )
